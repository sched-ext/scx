...
async def generate_veristat_matrix(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for kernel, csv_content in veristat_csvs.items():
            ...
            for row in reader:
                ...
                print(
                    f"    Row {row_count}: file_name='{file_name}', verdict='{verdict}'"
                )
        ...
        async def generate_readable_output(result):
            ...
            with tempfile.NamedTemporaryFile(mode="w", suffix=".csv") as f:
                ...
                result["output"] = await run_command_in_vm(
                    result["kernel"], ["veristat", "-R", f.name]
                )
                return result
