...
int rs_select_cpu(struct task_cpu_arg *input)
{
	 ...
	 * other tasks are queued. Since this function is invoked by the
	 * user-space scheduler, which will release the CPU shortly, there
	 * is no need to migrate the task elsewhere.
	 */
	if (cpu == bpf_get_smp_processor_id()) {
		bool is_busy = nr_running + nr_queued + nr_scheduled + 1 >= nr_online_cpus;

		if (!is_busy && !scx_bpf_dsq_nr_queued(cpu))
			goto out_release;
	}
