...
fn test_render_memory_view_with_swap_usage() {
    ...
    terminal
        .draw(|frame| {
            ...
            assert!(result.is_ok(), "render_memory_view with swap usage should succeed");
        })
        .unwrap();
}
...
fn test_render_memory_view_low_memory() {
    ...
    terminal
        .draw(|frame| {
            ...
            assert!(result.is_ok(), "render_memory_view with low memory should succeed");
        })
        .unwrap();
}
...
fn test_render_memory_summary_basic() {
    ...
    terminal
        .draw(|frame| {
            let area = frame.area();
            let result = MemoryRenderer::render_memory_summary(
                frame,
                area,
                &mem_stats,
                &keymap,
...
fn test_render_memory_summary_with_different_themes() {
    ...
    for theme in [AppTheme::Default, AppTheme::MidnightGreen, AppTheme::SolarizedDark] {
        terminal
            .draw(|frame| {
                let area = frame.area();
                let result = MemoryRenderer::render_memory_summary(
                ...
                assert!(result.is_ok(), "render_memory_summary with {:?} theme should succeed", theme);
            })
            .unwrap();
    }
}
...
fn test_render_memory_view_small_area() {
    ...
    terminal
        .draw(|frame| {
            ...
            assert!(result.is_ok(), "render_memory_view in small area should succeed");
        })
        .unwrap();
}
...
fn test_render_memory_view_zero_swap() {
    ...
    terminal
        .draw(|frame| {
            ...
            assert!(result.is_ok(), "render_memory_view with zero swap should succeed");
        })
        .unwrap();
}
...
fn test_render_memory_view_high_memory_usage() {
    ...
    terminal
        .draw(|frame| {
            ...
            assert!(result.is_ok(), "render_memory_view with high memory usage should succeed");
        })
        .unwrap();
}
