        ...
        match self.jited_line_info.binary_search(&ip) {
            ...
            Err(idx) => {
                ...
                if idx > 0 && idx <= self.jited_line_info.len() {
                    ...
                    if ip >= prev_addr && ip < prev_addr + 100 {
                        if prev_idx < self.line_info.len() {
...
impl BpfProgStats {
    ...
    fn extract_line_info(
        prog_fd: i32,
        prog_info: &BpfProgInfo,
    ) -> Result<(Vec<BpfLineInfo>, Vec<u64>, Vec<u64>, Vec<u32>)> {
        use std::mem;
        ...
        if prog_info.nr_line_info > 0 {
            ...
            let mut new_prog_info = BpfProgInfo::default();
            new_prog_info.line_info = line_info_buf.as_mut_ptr() as u64;
            new_prog_info.nr_line_info = prog_info.nr_line_info;
            new_prog_info.line_info_rec_size = prog_info.line_info_rec_size;
        ...
        if prog_info.nr_jited_line_info > 0 {
            ...
            // Create a new prog_info structure with the buffer pointers set
            let mut new_prog_info = BpfProgInfo::default();
            new_prog_info.jited_line_info = jited_line_info_buf.as_mut_ptr() as u64;
            new_prog_info.nr_jited_line_info = prog_info.nr_jited_line_info;
            new_prog_info.jited_line_info_rec_size = prog_info.jited_line_info_rec_size;
        ...
        if prog_info.nr_jited_ksyms > 0 {
            ...
            let mut new_prog_info = BpfProgInfo::default();
            new_prog_info.jited_ksyms = ksyms_buf.as_mut_ptr() as u64;
            new_prog_info.nr_jited_ksyms = prog_info.nr_jited_ksyms;
            new_prog_info.jited_func_lens = func_lens_buf.as_mut_ptr() as u64;
            new_prog_info.nr_jited_func_lens = prog_info.nr_jited_func_lens;
