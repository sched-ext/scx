...
use ratatui::{
    ...
    widgets::{
        ...
        Gauge, Paragraph, RenderDirection, Row, Scrollbar, ScrollbarOrientation, ScrollbarState,
...
impl<'a> App<'a> {
    ...
    fn render_default(&mut self, frame: &mut Frame) -> Result<()> {
        ...
        // Calculate dynamic sizing for network panel based on number of interfaces
        let num_interfaces = self.network_stats.interfaces.len();
        let min_network_lines = 3; // Header + border
        let max_network_lines = 12; // Reasonable maximum
        let lines_per_interface = 1;
        let network_lines = (min_network_lines + num_interfaces * lines_per_interface)
            .min(max_network_lines)
            .max(min_network_lines);

        // Calculate constraints based on available height
        let available_height = left.height as usize;
        let network_height = network_lines.min(available_height / 3); // Don't take more than 1/3
        let table_height = available_height.saturating_sub(network_height);

        let [left_top, left_bottom] = Layout::vertical([
            Constraint::Length(table_height as u16),
            Constraint::Length(network_height as u16),
        ])
        .areas(left);

        self.render_event(frame, right)?;
        self.render_table(frame, left_top)?;
        self.render_network_summary(frame, left_bottom)
    }
        ...
        let block = Block::bordered()
            .title_top({
                ...
                if network_key == "N" || network_key == "n" {
                    ...
                    Line::from(vec![
                        Span::styled(key_char, self.theme().text_important_color()),
                        Span::raw("etwork"),
                    ])
                    .style(self.theme().title_style())
                    .centered()
                } else {
                    Line::from(format!("Network (press {} for full view)", network_key))
                        .style(self.theme().title_style())
                        .centered()
                }
            })
        ...
        rows.push(
            Row::new(vec![
                ...
                Cell::from(if self.localize {
                    ...
                    format!("{}/s", total_delta_sent_errs)
                })
    ...
    fn render_interface_bytes_chart(
        ...
        let chart = Chart::new(datasets)
            .block(
                Block::bordered()
                    .title_top(
                        Line::from(format!("{} - Bits/s", interface))
                            .style(self.theme().title_style())
                            .centered(),
                    )
                    .border_type(BorderType::Rounded)
    ...
    fn render_interface_packets_chart(
        ...
        let chart = Chart::new(datasets)
            .block(
                Block::bordered()
                    .title_top(
                        Line::from(format!("{} - Packets/s", interface))
                            .style(self.theme().title_style())
                            .centered(),
                    )
                    .border_type(BorderType::Rounded)
