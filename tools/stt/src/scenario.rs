...
impl Flag {
    ...
    pub fn all() -> &'static [Flag] {
        &[
            ...
            Self::WorkStealing,
            Self::Rebalancing,
            Self::RejectMulticpuPinning,
            Self::CpuControllerDisabled,
        ]
    }
    ...
    fn requires(&self) -> &'static [Flag] {
        match self {
            Self::WorkStealing => &[Self::LlcAware],
            _ => &[],
        }
    }
...
fn generate_profiles(required: &[Flag], excluded: &[Flag]) -> Vec<FlagProfile> {
    let optional: Vec<Flag> = Flag::all()
        .iter()
        .copied()
...
impl Scenario {
    pub fn scheduler_args(&self, parent_cgroup: &str, profile: &FlagProfile) -> Vec<String> {
        ...
        for a in self.extra_sched_args {
    ...
    pub fn profiles_with(&self, active: &[Flag]) -> Vec<FlagProfile> {
        ...
        for f in Flag::all() {
            if !active.contains(f) && !self.required_flags.contains(f) {
                excl.push(*f);
            }
        }
...
fn setup_cells(ctx: &Ctx, n: usize, wl: &WorkloadConfig) -> Result<Vec<WorkloadHandle>> {
    ...
    let handles: Result<Vec<_>> = (0..n)
        .map(|i| {
            let h = WorkloadHandle::spawn(wl)?;
            for t in h.tids() {
                ctx.cgroups.move_task(&format!("cell_{i}"), t)?;
            }
            Ok(h)
        })
        .collect();
...
fn custom_dynamic_remove(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    thread::sleep(ctx.duration / 2);
    for h in handles.drain(half..) {
        h.stop_and_collect();
    }
    for i in half..n {
        let _ = ctx.cgroups.remove_cell(&format!("cell_{i}"));
    }
    thread::sleep(ctx.duration / 2);
    Ok(collect_all(handles))
}
...
fn custom_cpuset_remove(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    let mut handles: Vec<_> = ["cell_0", "cell_1"]
        ...
        .map(|n| {
            ...
            for t in h.tids() {
                ctx.cgroups.move_task(n, t).unwrap();
            }
            h
        })
        .collect();
...
fn custom_cpuset_change(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    ctx.cgroups.create_cell("cell_0")?;
    ctx.cgroups
        .set_cpuset("cell_0", &all[..q * 2].iter().copied().collect())?;
    ctx.cgroups.create_cell("cell_1")?;
    ctx.cgroups
        .set_cpuset("cell_1", &all[q * 2..last].iter().copied().collect())?;
    thread::sleep(Duration::from_secs(3));
    let wl = dfl_wl(ctx);
    let mut handles: Vec<_> = ["cell_0", "cell_1"]
        .iter()
    ...
    thread::sleep(t);
    ctx.cgroups
        .set_cpuset("cell_0", &all[..q].iter().copied().collect())?;
    ctx.cgroups
        .set_cpuset("cell_1", &all[q..last].iter().copied().collect())?;
    thread::sleep(t);
    ctx.cgroups
        .set_cpuset("cell_0", &all[..q * 3].iter().copied().collect())?;
    ctx.cgroups
        .set_cpuset("cell_1", &all[q * 3..last].iter().copied().collect())?;
    thread::sleep(t);
    Ok(collect_all(handles))
}
    ...
    for _ in 0..4 {
        ...
        for h in &handles {
            for idx in 0..h.tids().len() {
                ...
                let chosen: BTreeSet<usize> = all
                    .choose_multiple(&mut rand::thread_rng(), (all.len() / 2).max(1))
                    .copied()
                    .collect();
...
fn custom_host_stress(ctx: &Ctx) -> Result<VerifyResult> {
    let handles = setup_cells(ctx, 2, &dfl_wl(ctx))?;
    let mut host = WorkloadHandle::spawn(&WorkloadConfig {
...
fn custom_many_cells(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    for i in 0..n {
        ...
        ctx.cgroups.create_cell(&name)?;
        ctx.cgroups
            .set_cpuset(&name, &[all[i]].into_iter().collect())?;
    }
    ...
    for i in 0..n {
        let h = WorkloadHandle::spawn(&wl)?;
        for t in h.tids() {
            ctx.cgroups.move_task(&format!("many_{i}"), t)?;
        }
        handles.push(h);
    }
    thread::sleep(ctx.duration);
    ...
    for i in 0..n {
        let _ = ctx.cgroups.remove_cell(&format!("many_{i}"));
    }
    Ok(r)
}
...
fn custom_cell_exhaustion(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    for i in 0..n {
        ...
        ctx.cgroups.create_cell(&name)?;
        ctx.cgroups
            .set_cpuset(&name, &[all[i % all.len()]].into_iter().collect())?;
    }
    thread::sleep(Duration::from_secs(1));
    let half = n / 2;
    for i in 0..half {
        let _ = ctx.cgroups.remove_cell(&format!("exhaust_{i}"));
    }
    thread::sleep(Duration::from_secs(1));
    let wl = WorkloadConfig {
        num_workers: 1,
    ...
    for i in 0..half {
        let _ = ctx.cgroups.remove_cell(&format!("reuse_{i}"));
    }
    Ok(r)
}
...
fn custom_nested_move(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    let h = WorkloadHandle::spawn(&wl)?;
    for t in h.tids() {
        ctx.cgroups.move_task("cell_0/sub", t)?;
    }
    ...
    thread::sleep(interval);
    for t in h.tids() {
        let _ = ctx.cgroups.move_task("cell_0", t);
    }
    thread::sleep(interval);
    for t in h.tids() {
        let _ = ctx.cgroups.move_task("cell_1/sub", t);
    }
    thread::sleep(interval);
    for t in h.tids() {
        let _ = ctx.cgroups.move_task("cell_1", t);
    }
    thread::sleep(interval);
...
fn custom_nested_cpuset(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    for t in h.tids() {
        ctx.cgroups.move_task("cell_0/narrow", t)?;
    }
    h.start();
...
fn custom_cell_exclude(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    let cpu_wl = WorkloadConfig {
        num_workers: ctx.workers_per_cell,
        ..Default::default()
    };
    let io_wl = WorkloadConfig {
        num_workers: ctx.workers_per_cell,
...
fn custom_rebalancing_dynamic(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    for t in h.tids() {
        ctx.cgroups.move_task("cell_2", t)?;
    }
    h.start();
    handles.push(h);
    thread::sleep(ctx.duration / 2);
...
fn custom_no_ctrl_cgroup_move(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    for i in 0..9 {
        ...
        for &tid in &tids[..half] {
            let _ = ctx.cgroups.move_task(target, tid);
        }
    }
    thread::sleep(interval);
...
fn custom_borrowing_cpuset_change(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    ctx.cgroups.create_cell("cell_0")?;
    ctx.cgroups
        .set_cpuset("cell_0", &all[..mid].iter().copied().collect())?;
    ctx.cgroups.create_cell("cell_1")?;
    ctx.cgroups
        .set_cpuset("cell_1", &all[mid..last].iter().copied().collect())?;
    thread::sleep(Duration::from_secs(3));
...
fn custom_rebalancing_oscillate(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    let mut result = VerifyResult::pass();
    for i in 0..4 {
        let (heavy_cell, light_cell) = if i % 2 == 0 {
            ("cell_0", "cell_1")
        } else {
            ("cell_1", "cell_0")
        };
        let mut hh = WorkloadHandle::spawn(&WorkloadConfig {
            num_workers: ctx.workers_per_cell * 2,
            ..Default::default()
        })?;
        for t in hh.tids() {
            ctx.cgroups.move_task(heavy_cell, t)?;
        }
        let mut hl = WorkloadHandle::spawn(&WorkloadConfig {
            num_workers: 1,
            work_type: WorkType::YieldHeavy,
            ..Default::default()
        })?;
        for t in hl.tids() {
            ctx.cgroups.move_task(light_cell, t)?;
        }
        hh.start();
        hl.start();
        thread::sleep(phase);
...
fn custom_exclude_dynamic(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    for t in h_excl.tids() {
        ctx.cgroups.move_task("excluded_cell", t)?;
    }
...
fn custom_cpuset_disjoint(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    ctx.cgroups
        .set_cpuset("cell_0", &all[q * 2..last].iter().copied().collect())?;
    ctx.cgroups
        .set_cpuset("cell_1", &all[..q * 2].iter().copied().collect())?;
    thread::sleep(ctx.duration / 3);
    // Swap back
    ctx.cgroups
        .set_cpuset("cell_0", &all[..q * 2].iter().copied().collect())?;
...
fn custom_no_ctrl_borrowing(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    for i in 0..5 {
        thread::sleep(interval);
        let target = if i % 2 == 0 { "cell_1" } else { "cell_0" };
        for &tid in &tids[..2.min(tids.len())] {
            let _ = ctx.cgroups.move_task(target, tid);
        }
    }
    thread::sleep(interval);
...
fn custom_reject_pin_cpuset(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    for idx in 0..handles[0].tids().len() {
        if a_vec.len() >= 2 {
            let _ = handles[0].set_affinity(idx, &a_vec[..2].iter().copied().collect());
        }
    }
    for idx in 0..handles[1].tids().len() {
        if b_vec.len() >= 2 {
            let _ = handles[1].set_affinity(idx, &b_vec[..2].iter().copied().collect());
        }
    }
    thread::sleep(ctx.duration);
    Ok(collect_all(handles))
...
fn custom_dynamic_cpuset(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    ctx.cgroups.create_cell("cell_1")?;
    ctx.cgroups
        .set_cpuset("cell_1", &all[q..q * 2].iter().copied().collect())?;
    thread::sleep(Duration::from_secs(3));
    let wl = dfl_wl(ctx);
    let mut h0 = WorkloadHandle::spawn(&wl)?;
    for t in h0.tids() {
    ...
    ctx.cgroups.create_cell("cell_2")?;
    ctx.cgroups
        .set_cpuset("cell_2", &all[q * 2..last].iter().copied().collect())?;
...
fn custom_dynamic_borrowing(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    ctx.cgroups.create_cell("cell_2")?;
    let mut h2 = WorkloadHandle::spawn(&WorkloadConfig {
        num_workers: 4,
        ..Default::default()
    })?;
    for t in h2.tids() {
...
fn custom_nested_borrowing(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    for t in h0.tids() {
        ctx.cgroups.move_task("cell_0/sub_a", t)?;
    }
    let mut h1 = WorkloadHandle::spawn(&WorkloadConfig {
        num_workers: 2,
        work_type: WorkType::Bursty {
    ...
    for t in h1.tids() {
        ctx.cgroups.move_task("cell_1/sub_b", t)?;
    }
    h0.start();
    h1.start();
    thread::sleep(ctx.duration);
...
fn custom_mix_cpuset(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    for (i, n) in names.iter().enumerate() {
        ...
        ctx.cgroups
            .set_cpuset(n, &all[start..end].iter().copied().collect())?;
    }
    thread::sleep(Duration::from_secs(3));
...
fn custom_mix_dynamic(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    for t in h.tids() {
        ctx.cgroups.move_task("cell_3", t)?;
    }
    h.start();
    handles.push(h);
    thread::sleep(ctx.duration / 3);
...
fn custom_dispatch_contention(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    ctx.cgroups.create_cell("cell_0")?;
    ctx.cgroups
        .set_cpuset("cell_0", &all[..last].iter().copied().collect())?;
    thread::sleep(Duration::from_secs(3));
...
fn custom_vtime_contamination(ctx: &Ctx) -> Result<VerifyResult> {
    ...
    if all.len() < 12 {
        return Ok(VerifyResult {
            passed: true,
            details: vec!["skipped: need >=12 CPUs".into()],
            stats: Default::default(),
        });
    }
    ...
    let apply_disjoint = |cells: &[String], usable: &[usize]| -> Result<()> {
        ...
        for (i, name) in cells.iter().enumerate() {
            let start = i * chunk;
            let end = if i == cells.len() - 1 {
                usable.len()
            } else {
                (i + 1) * chunk
            };
            ctx.cgroups
                .set_cpuset(name, &usable[start..end].iter().copied().collect())?;
        }
        Ok(())
    };
    ...
    let apply_overlap = |cells: &[String], usable: &[usize], overlap_frac: f64| -> Result<()> {
        ...
        for (i, name) in cells.iter().enumerate() {
            let start = if i == 0 {
                ...
                (i * chunk).saturating_sub(overlap)
            };
            let end = if i == cells.len() - 1 {
                usable.len()
            } else {
                ((i + 1) * chunk + overlap).min(usable.len())
            };
            ctx.cgroups
                .set_cpuset(name, &usable[start..end].iter().copied().collect())?;
        }
...
pub fn all_scenarios() -> Vec<Scenario> {
    ...
    vec![
        ...
        s!(
            "proportional",
            "basic",
            "No cpusets, proportional CPU division",
            2,
            CpusetMode::None,
            dfl()
        ),
        s!(
            "cpuset_aligned",
            "basic",
            "Cpusets aligned to LLC boundaries",
            2,
            CpusetMode::LlcAligned,
            dfl()
        ),
        s!(
            "cpuset_misaligned",
            "basic",
            "Cpusets spanning LLC boundaries",
            2,
            CpusetMode::SplitMisaligned,
            dfl()
        ),
        ...
        Scenario {
            name: "borrowing_idle",
            category: "advanced",
            description: "One cell idle, other overloaded",
            required_flags: &[Flag::Borrowing],
            excluded_flags: &[],
            num_cells: 2,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![CellWork {
                workers: 16,
                ..Default::default()
            }], // only cell 0 gets workers
            action: Action::Steady,
            extra_sched_args: &[],
        },
        Scenario {
            name: "borrowing_contention",
            category: "advanced",
            description: "All cells loaded with borrowing",
            required_flags: &[Flag::Borrowing],
            excluded_flags: &[],
            num_cells: 2,
            cpuset_mode: CpusetMode::None,
        ...
        Scenario {
            name: "work_stealing",
            category: "advanced",
            description: "LLC-aware work stealing",
            required_flags: &[Flag::LlcAware, Flag::WorkStealing],
            excluded_flags: &[],
            num_cells: 1,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![CellWork {
        ...
        Scenario {
            name: "rebalancing",
            category: "advanced",
            description: "Demand-based CPU rebalancing",
            required_flags: &[Flag::Rebalancing],
            excluded_flags: &[],
            num_cells: 2,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![
        ...
        Scenario {
            name: "stall_detect",
            category: "stall",
            description: "Low watchdog + overloaded cell",
            required_flags: &[],
            excluded_flags: &[],
            num_cells: 1,
            cpuset_mode: CpusetMode::None,
        ...
        Scenario {
            name: "affinity_llc_aligned",
            category: "affinity",
            description: "LLC-aligned affinities",
            required_flags: &[Flag::LlcAware],
            excluded_flags: &[],
            num_cells: 2,
        ...
        Scenario {
            name: "affinity_mutation",
            category: "affinity",
            description: "Change affinities while running",
            required_flags: &[],
            excluded_flags: &[Flag::RejectMulticpuPinning],
            num_cells: 2,
            cpuset_mode: CpusetMode::None,
            cell_works: dfl(),
            action: Action::Custom(custom_affinity_mutation),
            extra_sched_args: &[],
        },
        // Custom scenarios
        custom!(
            "dynamic_add",
            "dynamic",
            "Add cells while running",
            custom_dynamic_add
        ),
        custom!(
            "dynamic_remove",
            "dynamic",
            "Remove cells while running",
            custom_dynamic_remove
        ),
        custom!(
            "dynamic_churn",
            "dynamic",
        ...
        custom!(
            "cpuset_add",
            "cpuset",
            "Add cpusets to existing cells",
            custom_cpuset_add
        ),
        custom!(
            "cpuset_remove",
            "cpuset",
            "Remove cpusets from cells",
            custom_cpuset_remove
        ),
        custom!(
            "cpuset_change",
            "cpuset",
            "Change cpusets on live cells",
            custom_cpuset_change
        ),
        custom!(
            "host_stress",
            "stress",
            "Host workers competing with cells",
            custom_host_stress
        ),
        custom!(
            "many_cells",
        ...
        custom!(
            "cell_exhaustion",
            "advanced",
            "Fill cells, remove, verify reuse",
            custom_cell_exhaustion
        ),
        custom!(
            "sched_mixed",
            "sched_class",
        ...
        custom!(
            "io_sync",
            "sched_class",
            "Sync IO mixed with CPU workers",
            custom_io_sync
        ),
        Scenario {
            name: "stall_recovery",
        ...
        custom!(
            ...
            custom_nested_cpuset
        ),
        // Cell exclusion
        Scenario {
            name: "cell_exclude",
            category: "advanced",
            description: "Excluded cgroup stays in cell 0",
            required_flags: &[],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_cell_exclude),
            extra_sched_args: &["--cell-exclude", "excluded_cell"],
        },
        // Borrowing + cpusets
        Scenario {
            name: "borrowing_cpuset",
            category: "advanced",
            description: "Borrowing with cpuset-constrained cells",
            required_flags: &[Flag::Borrowing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_borrowing_cpuset),
            extra_sched_args: &[],
        },
        // Rebalancing + cpusets
        Scenario {
            name: "rebalancing_cpuset",
            category: "advanced",
            description: "Rebalancing with cpuset-constrained cells",
            required_flags: &[Flag::Rebalancing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_rebalancing_cpuset),
            extra_sched_args: &[],
        },
        // Rebalancing + dynamic cell add
        Scenario {
            name: "rebalancing_dynamic",
            category: "advanced",
            description: "Rebalancing during cell add",
            required_flags: &[Flag::Rebalancing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_rebalancing_dynamic),
            extra_sched_args: &[],
        },
        // Borrowing + rebalancing combined
        Scenario {
            name: "borrowing_rebalancing",
            category: "advanced",
            description: "Borrowing + rebalancing together",
            required_flags: &[Flag::Borrowing, Flag::Rebalancing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_borrowing_rebalancing),
            extra_sched_args: &[],
        },
        ...
        Scenario {
            name: "reject_pin",
            category: "advanced",
            description: "Multi-CPU affinity rejection",
            required_flags: &[Flag::RejectMulticpuPinning],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_reject_pin),
            extra_sched_args: &[],
        },
        // CPU controller disabled: rapid cgroup moves
        Scenario {
            name: "no_ctrl_cgroup_move",
            category: "advanced",
            description: "Rapid cgroup moves in no-ctrl mode",
            required_flags: &[Flag::CpuControllerDisabled],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_no_ctrl_cgroup_move),
            extra_sched_args: &[],
        },
        // Borrowing + cpuset change (borrowed CPU disappears from lender)
        Scenario {
            name: "borrowing_cpuset_change",
            category: "advanced",
            description: "Cpuset change while borrowing",
            required_flags: &[Flag::Borrowing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_borrowing_cpuset_change),
            extra_sched_args: &[],
        },
        // Rebalancing oscillation
        Scenario {
            name: "rebalancing_oscillate",
            category: "advanced",
            description: "Alternating load tests rebalancing stability",
            required_flags: &[Flag::Rebalancing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_rebalancing_oscillate),
            extra_sched_args: &[],
        },
        ...
        Scenario {
            name: "exclude_cpuset",
            category: "advanced",
            description: "Excluded cell + cpuset cells",
            required_flags: &[],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_exclude_cpuset),
            extra_sched_args: &["--cell-exclude", "excluded_cell"],
        },
        ...
        Scenario {
            ...
            category: "advanced",
            description: "Excluded cell + dynamic cell changes",
            required_flags: &[],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_exclude_dynamic),
            extra_sched_args: &["--cell-exclude", "excluded_cell"],
        },
        ...
        Scenario {
            name: "no_ctrl_nested",
            category: "advanced",
            description: "Nested cgroups in no-ctrl mode",
            required_flags: &[Flag::CpuControllerDisabled],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_no_ctrl_nested),
            extra_sched_args: &[],
        },
        // Cpuset disjoint swap (vtime domain discontinuity)
        Scenario {
            name: "cpuset_disjoint",
            category: "cpuset",
            description: "Cpuset swapped to disjoint CPU range",
            required_flags: &[],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_cpuset_disjoint),
            extra_sched_args: &[],
        },
        // IO + borrowing
        Scenario {
            name: "io_borrowing",
            category: "advanced",
            description: "IO cell frees CPUs for borrowing",
            required_flags: &[Flag::Borrowing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_io_borrowing),
            extra_sched_args: &[],
        },
        // Rebalancing with many cells + asymmetric demand
        Scenario {
            name: "rebalancing_many",
            category: "advanced",
            description: "Rebalancing with 4 asymmetric cells",
            required_flags: &[Flag::Rebalancing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_rebalancing_many),
            extra_sched_args: &[],
        },
        // Borrowing + overlapping cpusets
        Scenario {
            name: "borrowing_overlap",
            category: "advanced",
            description: "Borrowing with overlapping cpusets",
            required_flags: &[Flag::Borrowing],
            excluded_flags: &[],
            num_cells: 3,
        ...
        Scenario {
            name: "rebalancing_overlap",
            category: "advanced",
            description: "Rebalancing with overlapping cpusets",
            required_flags: &[Flag::Rebalancing],
            excluded_flags: &[],
            num_cells: 3,
            cpuset_mode: CpusetMode::Overlap(0.5),
        ...
        Scenario {
            name: "borrowing_bursty",
            category: "advanced",
            description: "Borrowing with bursty workload",
            required_flags: &[Flag::Borrowing],
            excluded_flags: &[],
            num_cells: 2,
            cpuset_mode: CpusetMode::None,
        ...
        Scenario {
            name: "rebalancing_shift",
            category: "advanced",
            description: "Rebalancing with bursty load shift",
            required_flags: &[Flag::Rebalancing],
            excluded_flags: &[],
            num_cells: 2,
            cpuset_mode: CpusetMode::None,
        ...
        Scenario {
            name: "vtime_contamination",
            category: "stress",
            description: "Hot/cold cells trigger cross-cell vtime contamination",
            required_flags: &[
                Flag::Borrowing,
                Flag::Rebalancing,
                Flag::CpuControllerDisabled,
            ],
            excluded_flags: &[Flag::RejectMulticpuPinning],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_vtime_contamination),
            extra_sched_args: &["--cell-exclude", "excluded_cell"],
        },
        // Borrowing + rebalancing + cpusets
        Scenario {
            name: "borrow_rebal_cpuset",
            category: "interaction",
            description: "Borrowing + rebalancing + cpusets",
            required_flags: &[Flag::Borrowing, Flag::Rebalancing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_borrow_rebal_cpuset),
            extra_sched_args: &[],
        },
        // Borrowing + rebalancing + overlapping cpusets
        Scenario {
            name: "borrow_rebal_overlap",
            category: "interaction",
            description: "Borrowing + rebalancing + overlapping cpusets",
            required_flags: &[Flag::Borrowing, Flag::Rebalancing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_borrow_rebal_overlap),
            extra_sched_args: &[],
        },
        // Cell exclude + borrowing
        Scenario {
            name: "exclude_borrowing",
            category: "interaction",
            description: "Excluded cell borrows from idle cells",
            required_flags: &[Flag::Borrowing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_exclude_borrowing),
            extra_sched_args: &["--cell-exclude", "excluded_cell"],
        },
        // Cell exclude + rebalancing
        Scenario {
            name: "exclude_rebalancing",
            category: "interaction",
            description: "Excluded cell load vs rebalancing",
            required_flags: &[Flag::Rebalancing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_exclude_rebalancing),
            extra_sched_args: &["--cell-exclude", "excluded_cell"],
        },
        // No-ctrl + borrowing
        Scenario {
            name: "no_ctrl_borrowing",
            category: "interaction",
            description: "Polling detection + borrowing + cgroup moves",
            required_flags: &[Flag::CpuControllerDisabled, Flag::Borrowing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_no_ctrl_borrowing),
            extra_sched_args: &[],
        },
        // No-ctrl + cpusets
        Scenario {
            name: "no_ctrl_cpuset",
            category: "interaction",
            description: "Cpuset changes in no-ctrl mode",
            required_flags: &[Flag::CpuControllerDisabled],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_no_ctrl_cpuset),
            extra_sched_args: &[],
        },
        // No-ctrl + rebalancing
        Scenario {
            name: "no_ctrl_rebalancing",
            category: "interaction",
            description: "Rebalancing with polling detection",
            required_flags: &[Flag::CpuControllerDisabled, Flag::Rebalancing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_no_ctrl_rebalancing),
            extra_sched_args: &[],
        },
        // Reject-pin + cpusets
        Scenario {
            name: "reject_pin_cpuset",
            category: "interaction",
            description: "Reject pin within cpuset cells",
            required_flags: &[Flag::RejectMulticpuPinning],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_reject_pin_cpuset),
            extra_sched_args: &[],
        },
        // Dynamic + cpusets
        Scenario {
            name: "dynamic_cpuset",
            category: "interaction",
            description: "Add/remove cpuset-constrained cells",
            required_flags: &[],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_dynamic_cpuset),
            extra_sched_args: &[],
        },
        // Dynamic + borrowing
        Scenario {
            name: "dynamic_borrowing",
            category: "interaction",
            description: "Add cell while borrowing active",
            required_flags: &[Flag::Borrowing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_dynamic_borrowing),
            extra_sched_args: &[],
        },
        // Nested + borrowing
        Scenario {
            name: "nested_borrowing",
            category: "interaction",
            description: "Nested cgroups with borrowing",
            required_flags: &[Flag::Borrowing],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_nested_borrowing),
            extra_sched_args: &[],
        },
        // Dispatch contention
        custom!(
            "dispatch_contention",
            "stress",
            "Cell DSQ contention starving per-CPU DSQ",
            custom_dispatch_contention
        ),
        // Workload variety - no flags
        custom!(
            "mix_no_flags",
            "stress",
            "All workload types, no flags",
            custom_mix_no_flags
        ),
        custom!(
            "mix_cpuset",
            "stress",
            "All workload types + cpusets",
            custom_mix_cpuset
        ),
        custom!(
            "mix_dynamic",
            "stress",
            "All workload types + dynamic cells",
            custom_mix_dynamic
        ),
        ...
        Scenario {
            name: "mix_allflags",
            category: "stress",
            description: "All workloads + all flags",
            required_flags: &[
                Flag::Borrowing,
                Flag::Rebalancing,
                Flag::CpuControllerDisabled,
                Flag::RejectMulticpuPinning,
            ],
            excluded_flags: &[],
            num_cells: 0,
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_mix_allflags),
            extra_sched_args: &["--cell-exclude", "excluded_cell"],
        },
        Scenario {
            name: "mix_allflags_cpuset",
            category: "stress",
            description: "All workloads + all flags + cpusets",
            required_flags: &[
                Flag::Borrowing,
                Flag::Rebalancing,
                Flag::CpuControllerDisabled,
            ...
            cpuset_mode: CpusetMode::None,
            cell_works: vec![],
            action: Action::Custom(custom_mix_allflags_cpuset),
            extra_sched_args: &["--cell-exclude", "excluded_cell"],
        },
        Scenario {
            name: "mix_allflags_dynamic",
            category: "stress",
            description: "All workloads + all flags + dynamic",
            required_flags: &[
                Flag::Borrowing,
                Flag::Rebalancing,
                Flag::CpuControllerDisabled,
...
mod tests {
    ...
    fn profile_name_with_flags() {
        let p = FlagProfile {
            flags: vec![Flag::LlcAware, Flag::Borrowing],
        };
    ...
    fn profile_args_maps_to_cli() {
        let p = FlagProfile {
            flags: vec![Flag::Borrowing, Flag::Rebalancing],
        };
        let args = p.args();
    ...
    fn generate_profiles_work_stealing_requires_llc() {
        ...
        for p in &profiles {
            assert!(
                p.flags.contains(&Flag::LlcAware),
                "WorkStealing without LlcAware: {:?}",
                p.flags
            );
        }
    }
    ...
    fn profiles_with_filters_correctly() {
        ...
        for p in &profiles {
            ...
            for f in &p.flags {
                assert!(
                    *f == Flag::Borrowing || Flag::Borrowing.requires().contains(f),
                    "unexpected flag {:?}",
                    f
                );
            }
