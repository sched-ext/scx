...
/*
 * The running average operations, included by ravg.bpf.c once per address
 * space: RAVG_AS qualifies the ravg_data pointer, RAVG_ARG tags the parameter
 * and RAVG_FN() names the function. The arena flavor carries the _arena suffix
 * and shares the documentation below. No include guard on purpose.
 */
 ...
 */
__weak
int RAVG_FN(ravg_accumulate)(struct ravg_data RAVG_AS RAVG_ARG *rd, u64 new_val, u64 now,
			     u32 half_life)
{
	...
	if (seq_delta > 0) {
		...
		rd->old = ravg_sat_add(rd->old, ravg_decay(rd->cur, seq_delta));
		/* clear */
		rd->cur = 0;
	}
	...
	if (seq_delta > 0) {
		...
		rd->old = ravg_sat_add(rd->old, rd->val * ravg_decay(dur, seq_delta));
		...
		if (seq_delta > 1) {
			...
			rd->old = ravg_sat_add(rd->old, rd->val * ravg_full_sum[idx]);
		}
 ...
 */
__weak
int RAVG_FN(ravg_scale)(struct ravg_data RAVG_AS RAVG_ARG *rd, u32 mult, u32 rshift)
{
	if (!rd)
		return 0;
 ...
 */
__weak
u64 RAVG_FN(ravg_read)(struct ravg_data RAVG_AS RAVG_ARG *rd, u64 now, u64 half_life)
{
	 ...
	 * synchronizing with the updater (we need to add a seq lock tho). The
	 * copy goes field by field: a struct assignment through an arena
	 * pointer can lose the address space on older clang, see ravg.h.
	 */
	trd.val = rd->val;
	trd.val_at = rd->val_at;
	trd.old = rd->old;
	trd.cur = rd->cur;
	ravg_accumulate(&trd, 0, now, half_life);
	...
	if (elapsed) {
		 ...
		 */
		u64 old = u64_x_u32_rshift(trd.old,
					   (1 << RAVG_FRAC_BITS) - progress / 2,
					   RAVG_FRAC_BITS);
		/*
