...
static u64 task_deadline(struct task_struct *p)
{
	...
	/*
	 * Return the task's deadline by subtracting the unused portion of the
	 * previous time slice from the task's virtual runtime (scaled by the
	 * task's weight).
	 *
	 * The idea is to shorten the deadline if the task didn't fully utilize
	 * its assigned time slice in its previous execution period.
	 */
	return p->scx.dsq_vtime - scale_inverse(p->scx.slice, p->scx.weight);
}
...
static inline u64 task_slice(struct task_struct *p)
{
	...
	/*
	 * If the task is classified as interactive, immediately refill its
	 * time slice budget as if it didn't use any during its last run
	 * period.
	 */
	if (is_task_interactive(p))
		p->scx.slice = slice_ns;

	return p->scx.slice + MAX(slice_ns / (nr_waiting + 1), slice_ns_min);
}
