...
int submit_task_ctx(struct task_struct *p, task_ctx __arg_arena *taskc, u32 cpu_id)
{
	...
	m->taskc_x.lat_cri = taskc->lcd.lat_cri;
	m->taskc_x.avg_lat_cri = sys_stat.avg_lat_cri;
	m->taskc_x.static_prio = get_nice_prio(p);
	m->taskc_x.rerunnable_interval_wall = time_delta(taskc->lcd.last_quiescent_clk, taskc->lcd.last_runnable_clk);
	m->taskc_x.resched_interval_wall = taskc->resched_interval_wall;
	m->taskc_x.run_freq = taskc->lcd.run_freq;
	m->taskc_x.avg_runtime_wall = taskc->lcd.avg_runtime;
	m->taskc_x.wait_freq = taskc->lcd.wait_freq;
	m->taskc_x.wake_freq = taskc->lcd.wake_freq;
