...
const volatile bool use_cpu_util;
...
/*
 * Try to dispatch a task directly if it's a critical system task.
 */
...
void BPF_STRUCT_OPS(tickless_enqueue, struct task_struct *p, u64 enq_flags)
{
	...
	/*
	 * Trigger a resched even on a primary CPU when a task that was not
	 * already running is enqueued.
	 */
	if (!scx_bpf_task_running(p)) {
		s32 cpu = pick_primary_cpu();
...
static bool dispatch_all_cpus(bool do_idle_smt)
{
	...
	bpf_for(cpu, 0, nr_cpu_ids) {
		...
		if (!use_cpu_util &&
		    !do_idle_smt && !bpf_cpumask_test_cpu(cpu, idle_cpumask))
			continue;
