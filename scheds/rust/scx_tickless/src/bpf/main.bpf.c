...
void BPF_STRUCT_OPS(tickless_dispatch, s32 cpu, struct task_struct *prev)
{
	if (is_primary_cpu(cpu)) {
		...
		/*
		 * If there are still tasks in the shared DSQ after
		 * distributing them to the tickless CPUs, start to consume
		 * them also from the primary CPUs.
		 */
		...
		/*
		 * This tickless CPU is about to go idle: kick and preempt
		 * a primary CPU so that it can dispatch a task here.
		 *
		 * We need to send a preemption IPI since primary CPUs may
		 * also run a task with infinite time slice if all the
