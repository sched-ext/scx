...
struct Opts {
    ...
    /// Allow to queue tasks on CPUs with low utilization (CPUs with a single task running and no
    /// other tasks queued), instead of always looking for idle CPU.
    ///
    /// This can help to improve task wakeup performance (producer/consumer workloads), but it can
    /// introduce OS noise and reduce performance isolation.
...
impl<'a> Scheduler<'a> {
    fn init(opts: &'a Opts, open_object: &'a mut MaybeUninit<OpenObject>) -> Result<Self> {
        ...
        skel.maps.rodata_data.use_cpu_util = opts.use_cpu_util;
