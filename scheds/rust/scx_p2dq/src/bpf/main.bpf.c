...
static void p2dq_dispatch_impl(s32 cpu, struct task_struct *prev)
{
	...
	if (!llcx && !(llcx = lookup_llc_ctx(cpuc->llc_id))) {
...
static int init_llc(u32 llc_index)
{
	...
	if (p2dq_config.llc_shards > 0) {
		llcx->nr_shards = min(p2dq_config.llc_shards, MAX_LLC_SHARDS);
...
static s32 p2dq_init_impl()
{
	...
	bpf_for(i, 0, topo_config.nr_cpus) {
		...
		if (p2dq_config.llc_shards > 0 && llcx->nr_shards > 0) {
