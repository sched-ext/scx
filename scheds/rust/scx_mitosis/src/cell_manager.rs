...
impl CellManager {
    ...
    pub fn compute_cpu_assignments(&self) -> Result<Vec<(u32, Cpumask)>> {
        ...
        let num_recipients = all_cell_ids.len() + 1; // +1 for cell 0
        ...
        // Cell 0 gets its share plus remainder
        ...
        // Other cells get equal shares
        for cell_id in all_cell_ids.iter().copied() {
        ...
        // Check all user cells
        for &cell_id in &all_cell_ids {
            if !cell_cpus.contains_key(&cell_id)
                || cell_cpus.get(&cell_id).map_or(true, |m| m.weight() == 0)
            {
                bail!(
