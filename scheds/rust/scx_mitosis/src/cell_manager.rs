...
impl CellManager {
    ...
    fn reconcile_cells(&mut self) -> Result<(Vec<(u64, u32)>, Vec<u32>)> {
        ...
        self.cells.retain(|&cgid, info| {
            if current_paths.contains(&info.cgroup_path) {
                true
            } else {
                info!(
                    "Destroyed cell {} for cgroup {} (cgid={})",
                    info.cell_id,
                    info.cgroup_path.display(),
                    cgid
                );
    ...
    pub fn compute_cpu_assignments(&self) -> Result<Vec<(u32, Cpumask)>> {
        ...
        // - Unclaimed: no cpuset claims it -> goes to cell 0 + unpinned cells
        ...
        // Phase 4: Distribute unclaimed CPUs among cell 0 + unpinned cells
        let mut unpinned_cells: Vec<u32> = self
            .cells
            .values()
            .filter(|info| info.cpuset.is_none())
            .map(|info| info.cell_id)
            .collect();
        unpinned_cells.sort(); // Deterministic ordering

        // Recipients are cell 0 plus all unpinned cells
        let num_recipients = unpinned_cells.len() + 1; // +1 for cell 0
        let nr_unclaimed = unclaimed_cpus.len();

        if nr_unclaimed > 0 {
            let cpus_per_recipient = nr_unclaimed / num_recipients;
            let remainder = nr_unclaimed % num_recipients;

            let mut cpu_iter = unclaimed_cpus.into_iter();

            // Cell 0 gets its share plus any remainder
            let cell0_count = cpus_per_recipient + remainder;
            for _ in 0..cell0_count {
                if let Some(cpu) = cpu_iter.next() {
                    cell_cpus
                        .entry(0)
                        .or_insert_with(Cpumask::new)
            ...
            // Unpinned cells get equal shares
            for cell_id in &unpinned_cells {
                for _ in 0..cpus_per_recipient {
                    if let Some(cpu) = cpu_iter.next() {
                        cell_cpus
                            .entry(*cell_id)
                            .or_insert_with(Cpumask::new)
        ...
        let total_cells = self.cells.len() + 1; // +1 for cell 0

        // Check cell 0
        if !cell_cpus.contains_key(&0) || cell_cpus.get(&0).map_or(true, |m| m.weight() == 0) {
            bail!(
                "Cell 0 has no CPUs assigned (nr_cpus={}, num_cells={})",
                self.nr_cpus,
                total_cells
            );
        }
        ...
        for cell_info in self.cells.values() {
            if !cell_cpus.contains_key(&cell_info.cell_id)
                || cell_cpus
                    .get(&cell_info.cell_id)
    ...
    pub fn get_cell_assignments(&self) -> Vec<(u64, u32)> {
        self.cells
            .values()
            .map(|info| (info.cgid, info.cell_id))
            .collect()
    }
...
impl CellManager {
    ...
    fn get_cell_ids(&self) -> Vec<u32> {
        self.cells.values().map(|c| c.cell_id).collect()
    }
