...
impl ConfiguredCells {
    ...
    /// Cgroup path of `cell_id` relative to the cgroup root, or "/" for the
    /// root cell and for ids that no longer map to a matched cgroup.
    pub fn cgroup_path_for_cell(&self, cell_id: u32) -> String {
        self.path_cell_ids
            .iter()
            .find(|&(_, &id)| id == cell_id)
            .map(|(path, _)| {
                path.strip_prefix(&self.cgroup_root)
