...
struct Scheduler<'a> {
    ...
    topo: Topology,        // Map of core ids -> CPU ids
...
impl<'a> Scheduler<'a> {
    fn init(opts: &Opts) -> Result<Self> {
        ...
        let topo = Topology::builder().build().expect("Failed to build CPU topology");
    ...
    fn nr_idle_cpus(&mut self) -> usize {
        ...
        for core_id in 0..self.topo.nr_cores() {
            let mut all_idle = true;
            for cpu_id in self.topo.core_cpus(core_id) {
