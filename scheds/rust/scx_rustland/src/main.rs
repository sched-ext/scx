...
impl AIModel {
    ...
    pub fn predict(&mut self, name: &str, weight: u64) -> u64 {
        ...
        let pred: f64 = f64::from(output.double_value(&[0, 0])) * self.y_max as f64;

        // Clamp output.
        const SLICE_MIN: u64 = 10 * NSEC_PER_USEC;
        const SLICE_MAX: u64 = 50 * NSEC_PER_MSEC;

        // Catch overflows and wrong predictions.
        let slice = if pred < 0.0 {
            SLICE_MAX
        } else {
            (pred as u64).clamp(SLICE_MIN, SLICE_MAX)
        };
...
impl<'a> Scheduler<'a> {
    ...
    fn dispatch_task(&mut self) -> bool {
        ...
        // Attempt to select an idle CPU for the task.
        let selected_cpu = self
            .bpf
            .select_cpu(task.qtask.pid, task.qtask.cpu, task.qtask.flags);

        dispatched_task.cpu = if selected_cpu >= 0 {
            // Found an idle CPU, dispatch there.
            selected_cpu
        } else if self.opts.percpu_local && task.qtask.nr_cpus_allowed == 1 {
            // If percpu_local is enabled, keep dispatching per-CPU tasks to their CPU directly.
            task.qtask.cpu
        } else {
            // No idle CPU found, fall back to the first CPU available.
            RL_CPU_ANY
        };
