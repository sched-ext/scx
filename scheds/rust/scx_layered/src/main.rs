...
impl<'a, 'b> Scheduler<'a, 'b> {
    ...
    fn init(opts: &Opts, layer_specs: &'b Vec<LayerSpec>, log_recorder: Arc<LogRecorder>) -> Result<Self> {
...
fn main() -> Result<()> {
    ...
    loop {
        let mut sched = Scheduler::init(&opts, &layer_config.specs, log_recorder.clone())?;
