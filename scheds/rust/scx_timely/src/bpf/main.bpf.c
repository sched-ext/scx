 ...
 */
private(TIMELY) struct bpf_cpumask __kptr *primary_cpumask;
 ...
 */
s32 BPF_STRUCT_OPS(timely_select_cpu, struct task_struct *p, s32 prev_cpu, u64 wake_flags)
{
...
void BPF_STRUCT_OPS(timely_dispatch, s32 cpu, struct task_struct *prev)
{
...
void BPF_STRUCT_OPS(timely_running, struct task_struct *p)
{
	struct task_ctx *tctx;
 ...
 */
void BPF_STRUCT_OPS(timely_stopping, struct task_struct *p, bool runnable)
{
...
void BPF_STRUCT_OPS(timely_runnable, struct task_struct *p, u64 enq_flags)
{
...
void BPF_STRUCT_OPS(timely_enable, struct task_struct *p)
{
	/*
...
s32 BPF_STRUCT_OPS(timely_init_task, struct task_struct *p,
...
s32 BPF_STRUCT_OPS_SLEEPABLE(timely_init)
{
	struct bpf_timer *timer;
	int err, i;
	u32 key = 0;
...
void BPF_STRUCT_OPS(timely_exit, struct scx_exit_info *ei)
{
	UEI_RECORD(uei, ei);
}

SCX_OPS_DEFINE(timely_ops,
	       ...
	       .enable			= (void *)timely_enable,
	       .init_task		= (void *)timely_init_task,
	       .init			= (void *)timely_init,
	       .exit			= (void *)timely_exit,
