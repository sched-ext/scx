...
{
	u64 nr_wait = scx_bpf_dsq_nr_queued(cpu) + 1;

	return scale_by_task_weight(p, slice_ns / nr_wait);
}
 ...
 * skipped.
 */
static bool try_migrate(const struct task_struct *p, u64 enq_flags)
{
...
s32 BPF_STRUCT_OPS(beerland_select_cpu, struct task_struct *p, s32 prev_cpu, u64 wake_flags)
{
	...
	if (cpu < 0) {
		 ...
		 * saturation.
		 */
		cpu = prev_cpu != this_cpu &&
		      bpf_cpumask_test_cpu(this_cpu, p->cpus_ptr) ? this_cpu : prev_cpu;
	}

	/*
	 * Always dispatch directly to the target CPU, since the task will
	 * always use its assigned per-CPU DSQ and we can save some locking
	 * contention dispatching here in ops.select_cpu().
	 *
...
static bool dispatch_from_remote_cpu(s32 from_cpu)
{
	...
	/*
	 * Pick the task with the smallest vruntime across all the CPUs.
	 */
	bpf_for(cpu, 0, nr_cpu_ids) {
		...
		/*
		 * TODO: Rewrite this when we'll have an API to peek from a
		 * remote DSQ in a lockless way.
		 */
