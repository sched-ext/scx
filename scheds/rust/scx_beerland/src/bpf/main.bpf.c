...
{
	return task_apply_sleep_vlag(p, tctx);
}

/*
 * Return @p's vruntime including runtime accumulated since it started running.
 */
static u64 task_current_vtime(const struct task_struct *p)
{
 ...
 */
static bool should_preempt_curr(const struct task_struct *curr,
				u64 dl, bool is_sleep_wakeup)
{
	...
	curr_vtime = task_current_vtime(curr);
...
void BPF_STRUCT_OPS(beerland_enqueue, struct task_struct *p, u64 enq_flags)
{
	...
	if (curr) {
		preempt_wakeup = should_preempt_curr(curr, dl, is_wakeup);
		if (preempt_wakeup)
...
{
	u64 min_vtime = ULLONG_MAX, cpu, min_cpu, now;
