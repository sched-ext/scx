...
/// Adjust a threshold based on event rate. Direction:
/// - Too many events (rate > HIGH): threshold is too low -> increase threshold so fewer tasks qualify.
/// - Too few events (rate < LOW):  threshold is too high -> DECREASE threshold so more tasks qualify.
///
/// Returns the new threshold clamped to [base/100, base*10000] (and at least 1 when base is 0).
