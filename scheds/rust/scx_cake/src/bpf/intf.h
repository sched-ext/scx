...
#define CAKE_CPU_STATUS_EPOCH_SHIFT 16U
#define CAKE_CPU_STATUS_EPOCH_MASK 0xfULL
#define CAKE_CPU_STATUS_LATENCY_SHIFT 20U
#define CAKE_CPU_STATUS_LATENCY_MASK 0x3ULL
...
#define CAKE_CPU_LATENCY_UNKNOWN 0U
#define CAKE_CPU_LATENCY_LATENCY 1U
#define CAKE_CPU_LATENCY_FRAME 2U
#define CAKE_CPU_LATENCY_BULK 3U
 ...
 *
 * cake_trust_user is written by userspace at low cadence after the packed
 * confidence lanes prove a CPU is predictable. cake_trust_bpf is written by
 * BPF only on rare hard misses. Keeping ownership split avoids atomics in the
 * scheduler hot path: BPF reads a tiny policy word, and if the trusted claim
 * misses it blocks that generation until userspace cools down and promotes a
 * fresh one.
 */
#define CAKE_TRUST_FLAG_PREV_DIRECT (1U << 0)

enum cake_trust_demotion_reason {
	CAKE_TRUST_DEMOTE_NONE		  = 0,
	CAKE_TRUST_DEMOTE_PREV_CLAIM_MISS = 1,
	CAKE_TRUST_DEMOTE_MAX		  = 2,
};

struct cake_trust_user {
	u32 policy; /* CAKE_TRUST_FLAG_* enabled by userspace */
	u32 generation; /* Incremented by userspace on promotion */
} __attribute__((aligned(64)));
_Static_assert(sizeof(struct cake_trust_user) == 64,
	       "cake_trust_user must stay one cache line");

struct cake_trust_bpf {
	u32 blocked; /* CAKE_TRUST_FLAG_* blocked for generation */
...
/* Select cascade:
 *   1. Cake route prediction from packed per-CPU confidence
 *   2. Cake scoreboard probes over prev/fast-probe candidates
 *   3. Native kernel idle helper fallback for correctness and broad search
 *   4. Hybrid perf-ordered backup when compiled in
 *   5. Tunnel: all CPUs busy -> return prev_cpu for local enqueue */
...
enum cake_flow_flags {
	...
	CAKE_FLOW_YIELDER = 1
			    << 1, /* Task voluntarily yielded since last stop */
 ...
 * telemetry for the confidence/prediction path; release builds compile their
 * writers out. */
enum cake_accel_route_stat {
	CAKE_ACCEL_ROUTE_MAX = 7,
};

enum cake_accel_route_block {
	CAKE_ACCEL_BLOCK_INVALID_PREV  = 0,
	CAKE_ACCEL_BLOCK_AFFINITY      = 1,
	CAKE_ACCEL_BLOCK_KTHREAD       = 2,
	CAKE_ACCEL_BLOCK_ROUTE_LOW     = 3,
	CAKE_ACCEL_BLOCK_SELECT_LOW    = 4,
	CAKE_ACCEL_BLOCK_TRUST_LOW     = 5,
	CAKE_ACCEL_BLOCK_LOAD_SHOCK    = 6,
	CAKE_ACCEL_BLOCK_FLOOR_LOW     = 7,
	CAKE_ACCEL_BLOCK_OWNER_LOW     = 8,
	CAKE_ACCEL_BLOCK_PULL_LOW      = 9,
	CAKE_ACCEL_BLOCK_AUDIT	       = 10,
	CAKE_ACCEL_BLOCK_LATENCY_GATE  = 11,
	CAKE_ACCEL_BLOCK_UNKNOWN_ROUTE = 12,
	CAKE_ACCEL_ROUTE_BLOCK_MAX     = 13,
};

enum cake_accel_probe_outcome {
	CAKE_ACCEL_PROBE_HIT	     = 0,
	CAKE_ACCEL_PROBE_BUSY	     = 1,
	CAKE_ACCEL_PROBE_DIRTY	     = 2,
	CAKE_ACCEL_PROBE_SMT_BUSY    = 3,
	CAKE_ACCEL_PROBE_CLAIM_FAIL  = 4,
	CAKE_ACCEL_PROBE_CLAIM_SKIP  = 5,
	CAKE_ACCEL_PROBE_INVALID     = 6,
	CAKE_ACCEL_PROBE_OUTCOME_MAX = 7,
};

enum cake_accel_pull_mode {
	CAKE_ACCEL_PULL_MODE_PULL  = 0,
	CAKE_ACCEL_PULL_MODE_PROBE = 1,
	CAKE_ACCEL_PULL_MODE_SKIP  = 2,
	CAKE_ACCEL_PULL_MODE_MAX   = 3,
};

enum cake_accel_pull_probe {
	CAKE_ACCEL_PULL_PROBE_EMPTY = 0,
	CAKE_ACCEL_PULL_PROBE_WORK  = 1,
	CAKE_ACCEL_PULL_PROBE_MAX   = 2,
};

enum cake_accel_native_fallback {
	CAKE_ACCEL_NATIVE_ENTRY = 0,
...
struct cake_task_ctx {
	...
	struct {
		...
		u64 wake_reason_wait_ns[CAKE_WAKE_REASON_MAX -
					1]; /* Wait by wake path */
		u32 wake_reason_count[CAKE_WAKE_REASON_MAX -
				      1]; /* Samples by wake path */
		u32 wake_reason_max_us[CAKE_WAKE_REASON_MAX -
				       1]; /* Worst wait by wake path */
		...
		u64 home_place_wait_ns
			[CAKE_PLACE_CLASS_MAX]; /* Wait totals by home locality */
		u32 home_place_wait_count
			[CAKE_PLACE_CLASS_MAX]; /* Wait samples by home locality */
		u32 home_place_wait_max_us
			[CAKE_PLACE_CLASS_MAX]; /* Worst wait by home locality */
		...
		u16 cpu_run_count
			[CAKE_TELEM_MAX_CPUS]; /* 128 bytes: per-CPU run count */
	} telemetry;
#endif /* !CAKE_RELEASE */
} __attribute__((aligned(CAKE_TCTX_ALIGN)));
_Static_assert(sizeof(struct cake_task_ctx) == CAKE_TCTX_SIZE,
...
struct cake_stats {
	...
	u64 smt_wake_wait_ns
		[2]; /* Wake wait by sibling-active-at-run-start bucket */
	...
	u64 wake_reason_wait_all_count
		[CAKE_WAKE_REASON_MAX]; /* Full-pop wake-to-run samples by wake path (0 unused) */
	u64 wake_reason_wait_all_max_ns
		[CAKE_WAKE_REASON_MAX]; /* Full-pop worst wait by wake path (0 unused) */
	u64 wake_reason_wait_ns
		[CAKE_WAKE_REASON_MAX]; /* Wait sums by wake path (0 unused) */
	u64 wake_reason_wait_count
		[CAKE_WAKE_REASON_MAX]; /* Wait samples by wake path (0 unused) */
	u64 wake_reason_wait_max_ns
		[CAKE_WAKE_REASON_MAX]; /* Worst wait by wake path (0 unused) */
	u64 wake_reason_bucket_count
		[CAKE_WAKE_REASON_MAX]
		[CAKE_WAKE_BUCKET_MAX]; /* Wake-to-run buckets by wake path (0 unused) */
	u64 select_path_count
		[CAKE_SELECT_PATH_MAX]; /* Path chosen in select_cpu (0 unused) */
	u64 select_path_migration_count
		[CAKE_SELECT_PATH_MAX]; /* Debug: select_cpu choices that changed CPU by path */
	u64 select_reason_migration_count
		[CAKE_SELECT_REASON_MAX]; /* Debug: select_cpu choices that changed CPU by reason */
	u64 select_reason_wait_ns
		[CAKE_SELECT_REASON_MAX]; /* Wake-to-run wait after each select_cpu decision reason */
	u64 select_reason_wait_count
		[CAKE_SELECT_REASON_MAX]; /* Wait samples after each select_cpu decision reason */
	u64 select_reason_wait_max_ns
		[CAKE_SELECT_REASON_MAX]; /* Worst wait after each select_cpu decision reason */
	u64 select_reason_bucket_count
		[CAKE_SELECT_REASON_MAX]
		[CAKE_WAKE_BUCKET_MAX]; /* Wait buckets after each select_cpu decision reason */
	u64 select_reason_select_ns
		[CAKE_SELECT_REASON_MAX]; /* select_cpu overhead by decision reason */
	u64 select_reason_select_count
		[CAKE_SELECT_REASON_MAX]; /* select_cpu samples by decision reason */
	u64 select_reason_select_max_ns
		[CAKE_SELECT_REASON_MAX]; /* Worst select_cpu overhead by decision reason */
	u64 accel_route_attempt_count
		[CAKE_ACCEL_ROUTE_MAX]; /* Route-token prediction claim attempts by route */
	u64 accel_route_hit_count
		[CAKE_ACCEL_ROUTE_MAX]; /* Route-token prediction claim hits by route */
	u64 accel_route_miss_count
		[CAKE_ACCEL_ROUTE_MAX]; /* Route-token prediction claim misses by route */
	u64 accel_fast_attempt_count
		[CAKE_ACCEL_ROUTE_MAX]; /* Scoreboard fast-scan claim attempts by route slot */
	u64 accel_fast_hit_count
		[CAKE_ACCEL_ROUTE_MAX]; /* Scoreboard fast-scan claim hits by route slot */
	u64 accel_fast_miss_count
		[CAKE_ACCEL_ROUTE_MAX]; /* Scoreboard fast-scan claim misses by route slot */
	u64 accel_route_block_count
		[CAKE_ACCEL_ROUTE_BLOCK_MAX]; /* Why route prediction could not use the floor path */
	u64 accel_scoreboard_probe_count
		[CAKE_ACCEL_ROUTE_MAX]
		[CAKE_ACCEL_PROBE_OUTCOME_MAX]; /* Scoreboard probe outcomes by route slot */
	u64 accel_pull_mode_count
		[CAKE_ACCEL_PULL_MODE_MAX]; /* Dispatch pull shaping decisions */
	u64 accel_pull_probe_count
		[CAKE_ACCEL_PULL_PROBE_MAX]; /* Dispatch queue probes that were empty or had work */
	u64 accel_native_fallback_count
		[CAKE_ACCEL_NATIVE_MAX]; /* Native helper fallback entry and ABI branch calls */
	u64 accel_accounting_relaxed; /* Stopping callbacks that skipped owner-scoreboard accounting */
	u64 accel_accounting_audit; /* Stopping callbacks that audited owner-scoreboard accounting */
	...
	u64 home_place_wait_count
		[CAKE_PLACE_CLASS_MAX]; /* Wait samples by task-home locality */
	u64 home_place_wait_max_ns
		[CAKE_PLACE_CLASS_MAX]; /* Worst wait by task-home locality */
	u64 home_place_run_ns
		[CAKE_PLACE_CLASS_MAX]; /* First-run sums by task-home locality */
	u64 home_place_run_count
		[CAKE_PLACE_CLASS_MAX]; /* First-run samples by task-home locality */
	u64 home_place_run_max_ns
		[CAKE_PLACE_CLASS_MAX]; /* Worst first-run by task-home locality */
	u64 waker_place_wait_ns
		[CAKE_PLACE_CLASS_MAX]; /* Wait sums by waker locality */
	u64 waker_place_wait_count
		[CAKE_PLACE_CLASS_MAX]; /* Wait samples by waker locality */
	u64 waker_place_wait_max_ns
		[CAKE_PLACE_CLASS_MAX]; /* Worst wait by waker locality */
	...
	u64 wake_class_reason_count
		[CAKE_WAKE_CLASS_REASON_MAX]; /* Shadow class reason hits */
	u64 wake_class_transition_count
		[CAKE_WAKE_CLASS_MAX]
		[CAKE_WAKE_CLASS_MAX]; /* Per-CPU owner class transitions */
	u64 busy_preempt_shadow_count
		[CAKE_BUSY_PREEMPT_SHADOW_MAX]; /* Shadow busy-wake decision counts */
	u64 busy_preempt_shadow_wakee_class_count
		[CAKE_WAKE_CLASS_MAX]; /* Busy shadow by wakee class */
	u64 busy_preempt_shadow_owner_class_count
		[CAKE_WAKE_CLASS_MAX]; /* Busy shadow by owner class */
	...
	u64 strict_wake_class_sample_count
		[CAKE_WAKE_CLASS_MAX]; /* Stricter shadow wake class samples */
	u64 strict_wake_class_reason_count
		[CAKE_WAKE_CLASS_REASON_MAX]; /* Strict class reason hits */
	u64 strict_wake_class_transition_count
		[CAKE_WAKE_CLASS_MAX]
		[CAKE_WAKE_CLASS_MAX]; /* Strict owner transitions */
	u64 strict_busy_preempt_shadow_count
		[CAKE_BUSY_PREEMPT_SHADOW_MAX]; /* Strict busy-wake decision counts */
	u64 strict_busy_preempt_shadow_wakee_class_count
		[CAKE_WAKE_CLASS_MAX]; /* Strict busy shadow by wakee class */
	u64 strict_busy_preempt_shadow_owner_class_count
		[CAKE_WAKE_CLASS_MAX]; /* Strict busy shadow by owner class */
	...
	u64 strict_wake_class_wait_ns
		[CAKE_WAKE_CLASS_MAX]; /* Strict wake-class wait sums */
	u64 strict_wake_class_wait_count
		[CAKE_WAKE_CLASS_MAX]; /* Strict wake-class wait samples */
	u64 strict_wake_class_wait_max_ns
		[CAKE_WAKE_CLASS_MAX]; /* Strict wake-class worst wait */
	...
	u64 wake_target_hit_count
		[CAKE_WAKE_REASON_MAX]; /* Post-wake subset: first run landed on the selected CPU */
	u64 wake_target_miss_count
		[CAKE_WAKE_REASON_MAX]; /* Post-wake subset: first run landed on a different CPU */
	u64 wake_followup_same_cpu_count
		[CAKE_WAKE_REASON_MAX]; /* Post-wake subset: first continuation stayed on the same CPU */
	u64 wake_followup_migrate_count
		[CAKE_WAKE_REASON_MAX]; /* Post-wake subset: first continuation migrated away */
	u64 nr_wake_kick_observed
		[CAKE_KICK_KIND_MAX]; /* Wake kicks followed by an observed run */
	u64 nr_wake_kick_quick
		[CAKE_KICK_KIND_MAX]; /* Wake kicks that led to a run within 200us */
	u64 total_wake_kick_to_run_ns
		[CAKE_KICK_KIND_MAX]; /* Kick-to-run total latency by kick kind */
	u64 max_wake_kick_to_run_ns
		[CAKE_KICK_KIND_MAX]; /* Worst kick-to-run latency by kick kind */
	u64 wake_kick_bucket_count
		[CAKE_KICK_KIND_MAX]
		[CAKE_WAKE_BUCKET_MAX]; /* Kick-to-run buckets by kick kind */
} __attribute__((aligned(64)));

/* Current default values */
#define CAKE_DEFAULT_QUANTUM_NS (2 * 1000 * 1000) /* 2ms */
#define CAKE_DEFAULT_NEW_FLOW_BONUS_NS \
	(3 * 1000 * 1000) /* Reserved legacy constant */
...
#define AQ_BULK_CEILING_COMPILE_NS \
	(8 * 1000000) /* 8ms legacy compile ceiling */

/* Legacy preemption thresholds retained for compatibility. */
#define CAKE_PREEMPT_YIELDER_THRESHOLD_NS \
	(100 * 1000) /* 100µs legacy threshold */
