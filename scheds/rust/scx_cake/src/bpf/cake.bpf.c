 ...
 *     with vtime-local kept as local-placement signal/accounting
...
#ifndef CAKE_SERVICE_READINESS
#define CAKE_SERVICE_READINESS 0
#endif
#if CAKE_SERVICE_READINESS
#include "service_readiness_table.h"
#endif

/* Local CPU DSQs remain non-stealable. vtime-local is deliberately signal-only
 * in release builds: it updates/clamps task vtime and direct-inserts into
 * builtin local DSQs instead of storing runnable work in shared per-LLC DSQs. */
...
#endif
#ifndef CAKE_WOW_FRAME_BOOST_VALUE
#define CAKE_WOW_FRAME_BOOST_VALUE 0
#endif
...
#endif
#if defined(CAKE_RELEASE) && CAKE_QUEUE_POLICY_VALUE == CAKE_QUEUE_POLICY_LLC_VTIME
/* 2026-06-20: vtime-local is no longer a shared runnable queue in the default
 * release build. Treat vtime as an ordering/preemption signal, then hand work
 * directly to the selected CPU's builtin local DSQ. */
#define CAKE_LLC_VTIME_SIGNAL_LOCAL 1
#else
#define CAKE_LLC_VTIME_SIGNAL_LOCAL 0
#endif
#if defined(CAKE_RELEASE) && \
	(CAKE_QUEUE_POLICY_VALUE == CAKE_QUEUE_POLICY_LOCAL || \
	 CAKE_LLC_VTIME_SIGNAL_LOCAL)
/* Release paths that own runnable work in builtin per-CPU local DSQs.  The
 * vtime-local arm joins this path as a signal-only variant: it keeps vtime
 * accounting and hybrid frame wake ordering but does not store tasks in a
 * shared LLC DSQ. */
#define CAKE_RELEASE_LOCAL_DISPATCH 1
#else
#define CAKE_RELEASE_LOCAL_DISPATCH 0
#endif
#if CAKE_RELEASE_LOCAL_DISPATCH
/* Release-local builds deliberately keep normal runnable custody in the builtin
 * per-CPU local DSQs.  Custom side lanes remain available to older shared/debug
 * A/B builds, but release local and signal-local vtime-local compile them out so
 * vtime is an ordering signal rather than another queueing subsystem. */
#define CAKE_USE_CUSTOM_DSQ_LANES 0
#else
#define CAKE_USE_CUSTOM_DSQ_LANES 1
#endif
#ifndef CAKE_LOCALITY_EXPERIMENTS
#define CAKE_LOCALITY_EXPERIMENTS 0
#endif
#ifndef CAKE_FRAME_RESERVE_VALUE
#define CAKE_FRAME_RESERVE_VALUE 0
#endif
#ifndef CAKE_HAS_ENQ_KICK_IDLE
#define CAKE_HAS_ENQ_KICK_IDLE 0
#endif
#ifndef CAKE_SELECT_CPU_AND_COMPAT_FASTPATH
#define CAKE_SELECT_CPU_AND_COMPAT_FASTPATH 0
#endif
#ifndef CAKE_SELECT_CPU_AND_STRUCT_FASTPATH
#define CAKE_SELECT_CPU_AND_STRUCT_FASTPATH 0
#endif
#ifndef CAKE_RELEASE
...
/* Runtime-mutating machine state. These are deliberately BSS, not rodata:
 * policy caps, current frequency, EPP/governor, IRQ/RT pressure, boost, and
 * thermal behavior may change after cake attaches. */
volatile u64 cake_external_pressure_mask;
volatile u32 cpu_dynamic_perf_score[CAKE_MAX_CPUS];
volatile u32 cpu_dynamic_cap_flags[CAKE_MAX_CPUS];
volatile u32 cpu_scaling_max_freq_khz[CAKE_MAX_CPUS];
volatile u32 cpu_scaling_min_freq_khz[CAKE_MAX_CPUS];
volatile u32 cpu_scaling_cur_freq_khz[CAKE_MAX_CPUS];
...
#define CAKE_ENABLE_CORE_STEAL_BUSY_FALLBACK CAKE_USE_CUSTOM_DSQ_LANES
...
/* Stable per-CPU capability surface. The Rust loader hydrates these from ACPI
 * CPPC and cpufreq sysfs at scheduler startup. Hot paths should consume reduced
 * route tables or compact dynamic masks rather than walking all raw fields, but
 * keeping the raw values in rodata makes dump/debug/AB testing deterministic
 * and keeps future policy experiments from re-reading sysfs. */
const volatile u32 cpu_base_perf_score[CAKE_MAX_CPUS]		       = {};
const volatile u32 cpu_cppc_highest_perf[CAKE_MAX_CPUS]		       = {};
const volatile u32 cpu_cppc_nominal_perf[CAKE_MAX_CPUS]		       = {};
const volatile u32 cpu_cppc_lowest_perf[CAKE_MAX_CPUS]		       = {};
const volatile u32 cpu_cppc_lowest_nonlinear_perf[CAKE_MAX_CPUS]      = {};
const volatile u32 cpu_cppc_reference_perf[CAKE_MAX_CPUS]	       = {};
const volatile u32 cpuinfo_max_freq_khz[CAKE_MAX_CPUS]		       = {};
...
static __noinline __maybe_unused int scx_lfdeq_enqueue_remote(struct scx_lfdeq __arena *lfdeq, u64 pid)
{
	...
	u64 cycle = t >> LFDEQ_CAPACITY_SHIFT;
...
#define CAKE_COMM7(c0, c1, c2, c3, c4, c5, c6) \
	(((u64)(u8)(c0)) | (((u64)(u8)(c1)) << 8) | \
	 ...
	 (((u64)(u8)(c4)) << 32) | (((u64)(u8)(c5)) << 40) | \
	 (((u64)(u8)(c6)) << 48))
#define CAKE_COMM5(c0, c1, c2, c3, c4) \
	(((u64)(u8)(c0)) | (((u64)(u8)(c1)) << 8) | \
	 (((u64)(u8)(c2)) << 16) | (((u64)(u8)(c3)) << 24) | \
	 (((u64)(u8)(c4)) << 32))
...
#define CAKE_COMM3(c0, c1, c2) \
	(((u64)(u8)(c0)) | (((u64)(u8)(c1)) << 8) | \
	 (((u64)(u8)(c2)) << 16))
#define CAKE_COMM4(c0, c1, c2, c3) \
	(((u64)(u8)(c0)) | (((u64)(u8)(c1)) << 8) | \
	 (((u64)(u8)(c2)) << 16) | (((u64)(u8)(c3)) << 24))

#define CAKE_COMM_MASK2 0x0000ffffULL
#define CAKE_COMM_MASK3 0x00ffffffULL
#define CAKE_COMM_MASK4 0x00000000ffffffffULL
#define CAKE_COMM_MASK5 0x000000ffffffffffULL
...
#define CAKE_COMM_GAME_THREAD CAKE_COMM8('G', 'a', 'm', 'e', 'T', 'h', 'r', 'e')
#define CAKE_COMM_RENDER_THREAD CAKE_COMM8('R', 'e', 'n', 'd', 'e', 'r', 'T', 'h')
#define CAKE_COMM_TASKGRAPH_THREAD CAKE_COMM8('T', 'a', 's', 'k', 'G', 'r', 'a', 'p')
#define CAKE_COMM_POOL_THREAD CAKE_COMM8('P', 'o', 'o', 'l', 'T', 'h', 'r', 'e')
#define CAKE_COMM_FPSAIM_THREAD CAKE_COMM8('F', 'P', 'S', 'A', 'i', 'm', 'T', 'r')
#define CAKE_COMM_CHROME_THREAD CAKE_COMM7('C', 'h', 'r', 'o', 'm', 'e', '_')
#define CAKE_COMM_CRGPU_THREAD CAKE_COMM8('C', 'r', 'G', 'p', 'u', 'M', 'a', 'i')
#define CAKE_COMM_DXVK_THREAD CAKE_COMM4('d', 'x', 'v', 'k')
#define CAKE_COMM_WOW_EXE CAKE_COMM7('W', 'o', 'W', '.', 'e', 'x', 'e')
#define CAKE_COMM_VKD3D_PREFIX CAKE_COMM5('v', 'k', 'd', '3', 'd')
#define CAKE_COMM_AUDIO_THREAD CAKE_COMM8('A', 'u', 'd', 'i', 'o', 'T', 'h', 'r')
#define CAKE_COMM_AUDIO_MIXER CAKE_COMM8('A', 'u', 'd', 'i', 'o', 'M', 'i', 'x')
#define CAKE_COMM_FAUDIO_THREAD CAKE_COMM8('F', 'A', 'u', 'd', 'i', 'o', '_', 'A')
...
#if CAKE_GAME_DIAG
static __always_inline void
cake_game_diag_count_nfw_hit_comm(const struct task_struct *p, u32 cpu)
{
	u64 comm0 = cake_task_comm_word(p, 0);

	if (comm0 == CAKE_COMM_GAME_THREAD) {
		CAKE_GAME_DIAG_INC(cpu, nfw_hit_game_thread);
		return;
	}
	if (comm0 == CAKE_COMM_RENDER_THREAD) {
		CAKE_GAME_DIAG_INC(cpu, nfw_hit_render_thread);
		return;
	}
	if (comm0 == CAKE_COMM_TASKGRAPH_THREAD) {
		CAKE_GAME_DIAG_INC(cpu, nfw_hit_taskgraph_thread);
		return;
	}
	if (comm0 == CAKE_COMM_POOL_THREAD) {
		CAKE_GAME_DIAG_INC(cpu, nfw_hit_pool_thread);
		return;
	}
	if (comm0 == CAKE_COMM_FPSAIM_THREAD) {
		CAKE_GAME_DIAG_INC(cpu, nfw_hit_fpsaim_thread);
		return;
	}
	if ((comm0 & CAKE_COMM_MASK7) == CAKE_COMM_CHROME_THREAD) {
		CAKE_GAME_DIAG_INC(cpu, nfw_hit_chrome_thread);
		return;
	}
	if (comm0 == CAKE_COMM_CRGPU_THREAD) {
		CAKE_GAME_DIAG_INC(cpu, nfw_hit_crgpu_thread);
		return;
	}
	if ((comm0 & CAKE_COMM_MASK4) == CAKE_COMM_DXVK_THREAD) {
		CAKE_GAME_DIAG_INC(cpu, nfw_hit_dxvk_thread);
		return;
	}
	if (comm0 == CAKE_COMM_AUDIO_THREAD ||
	    comm0 == CAKE_COMM_AUDIO_MIXER ||
	    comm0 == CAKE_COMM_FAUDIO_THREAD) {
		CAKE_GAME_DIAG_INC(cpu, nfw_hit_audio_thread);
		return;
	}
	CAKE_GAME_DIAG_INC(cpu, nfw_hit_other_thread);
}
#endif
...
{
#if CAKE_RELEASE_LOCAL_DISPATCH && CAKE_LEAN_SCHED
	u32 idx = cpu & (CAKE_MAX_CPUS - 1);
	...
	/*
	 * cache_simple_state exists only for the non-domain-DRR stress service
	 * plane. Domain-DRR owns cache/stream lifecycle in domain_drr[] instead,
	 * so the latency reset has no cache-simple residue to scrub.
	 */
#if !CAKE_HAS_DOMAIN_DRR
	if (simple_state) {
...
static __always_inline void cake_publish_cpu_idle(u32 cpu)
{
	...
	CAKE_GAME_DIAG_INC(cpu, publish_idle_call);
	if (!(old & CAKE_CPU_STATUS_IDLE)) {
		...
		CAKE_GAME_DIAG_INC(cpu, publish_idle_write);
	} else {
		CAKE_GAME_DIAG_INC(cpu, publish_idle_noop);
	}
}
...
static __always_inline void cake_publish_cpu_running(u32 cpu, bool task_changed)
{
	...
	CAKE_GAME_DIAG_INC(cpu, publish_running_call);
	if (task_changed) {
		...
		if (!cake_status_same_visible_state(old, next)) {
			WRITE_ONCE(cpu_status[idx].flags, next);
			cake_scoreboard_summary_publish(cpu, next);
			CAKE_GAME_DIAG_INC(cpu, publish_running_write);
		} else {
			CAKE_GAME_DIAG_INC(cpu, publish_running_noop);
		}
		return;
	}
	if (old & CAKE_CPU_STATUS_IDLE) {
						   ...
						   u32 owner_avg_runtime_ns)
{
	...
	CAKE_GAME_DIAG_INC(cpu, publish_owner_call);
	...
	if (!cake_status_same_visible_state(old, next)) {
		WRITE_ONCE(cpu_status[idx].flags, next);
		cake_scoreboard_summary_publish(cpu, next);
		CAKE_GAME_DIAG_INC(cpu, publish_owner_write);
	} else {
		CAKE_GAME_DIAG_INC(cpu, publish_owner_noop);
	}
}
...
static __always_inline __maybe_unused bool
cake_nfw_prev_idle_sibling_clear(u32 target_cpu)
{
	return !cake_smt_latency_neighbor_busy(target_cpu);
}

/* Lean prev-claim (SCX_CAKE_NFW_LEAN_PREV): per-wake cost cut. The default
 * nfw warm-prev path gates the idle claim on a remote SMT-sibling status read
 * (cake_nfw_prev_idle_sibling_clear). That read costs a cross-core cacheline
 * fetch every wake, and when it reports the sibling busy it BLOCKS reusing the
 * cache-warm prev_cpu and forces the expensive fallback (select_cpu_dfl scan +
 * cold-core migration). Under the knob we drop the sibling gate and take prev
 * whenever it is idle (single test_and_clear), trading a little SMT-contention
 * latency for a cheaper wake and more cache warmth — an avg-fps lever. */
#ifndef CAKE_NFW_LEAN_PREV
#define CAKE_NFW_LEAN_PREV 0
#endif
#ifndef CAKE_NFW_STRICT_SIBLING
#define CAKE_NFW_STRICT_SIBLING 0
#endif
#if CAKE_NFW_LEAN_PREV
#define cake_nfw_prev_take_ok(cpu) (true)
#elif CAKE_NFW_STRICT_SIBLING
/* Strict full-idle-core prev (SCX_CAKE_NFW_STRICT_SIBLING): take the warm
 * prev_cpu only if its SMT sibling is FULLY idle — reject even dxvk/bulk
 * siblings (the default latency check ignores them, which is why cake parks
 * GameThread/RenderThread on dxvk-busy siblings ~12x more than EEVDF). A
 * busy-sibling prev falls through to the builtin idle pick, which prefers
 * whole idle cores like EEVDF's select_idle_core — driving SMT contention
 * down toward EEVDF's ~0.3%. */
#define cake_nfw_prev_take_ok(cpu) (!cake_smt_any_neighbor_busy(cpu))
#else
#define cake_nfw_prev_take_ok(cpu) cake_nfw_prev_idle_sibling_clear(cpu)
#endif
...
static __noinline __maybe_unused void
dsq_insert_wrapper(struct task_struct *p, u64 dsq_id, u64 slice,
		   u64 enq_flags);
static __always_inline void cake_dsq_insert(struct task_struct *p, u64 dsq_id,
					    u64 slice, u64 enq_flags);
static __noinline void cake_clamp_wakeup_vtime(struct task_struct *p,
					       u32		   target_cpu);
#if CAKE_LLC_VTIME_SIGNAL_LOCAL
static __always_inline void cake_insert_llc_vtime(struct task_struct *p,
						  u64 enq_flags,
...
#if CAKE_SERVICE_READINESS
static __always_inline u8
cake_service_readiness_service_class(const struct task_struct *p,
				     u32 service_kind,
				     bool default_bulk_protected)
{
	...
	if (service_kind == CAKE_TASK_SERVICE_STRESS_FUTEX ||
	    service_kind == CAKE_TASK_SERVICE_SCHBENCH)
		return 1; /* futex / schbench readiness class */
	...
	if (service_kind == CAKE_TASK_SERVICE_STRESS_CACHE ||
	    service_kind == CAKE_TASK_SERVICE_STRESS_MEMCPY)
		return 5; /* cache/memcpy guardrail */
	if (default_bulk_protected)
		return 6; /* bulk guardrail */
	if (service_kind == CAKE_TASK_SERVICE_NONE)
		return 0; /* none */
	return 7; /* other */
}
...
static __always_inline u8
cake_service_readiness_target_class(u64 target_status, u32 target_cpu,
				    u32 enqueue_cpu)
{
	if (target_status & CAKE_CPU_STATUS_IDLE)
		return 0; /* idle */
	if (target_cpu == enqueue_cpu)
		return 2; /* local-busy */
	if (target_status & CAKE_CPU_STATUS_PREEMPT_WAKE)
		return 3; /* remote/preemptable busy */
	return 1; /* busy */
}

static __always_inline u8
cake_service_readiness_owner_class(u64 target_status,
				   bool default_bulk_protected)
{
	if (default_bulk_protected)
		return 2; /* bulk */
...
static __always_inline u8
cake_service_readiness_index_for_wake(const struct task_struct *p,
				      u64 target_status, u32 target_cpu,
				      u32 enqueue_cpu, u32 service_kind,
				      bool default_bulk_protected)
{
	u8 service = cake_service_readiness_service_class(
		p, service_kind, default_bulk_protected);
	u8 target = cake_service_readiness_target_class(target_status,
							target_cpu,
							enqueue_cpu);
	u8 owner = cake_service_readiness_owner_class(target_status,
						      default_bulk_protected);
	u8 fairness_due =
		default_bulk_protected ||
		!!(target_status & CAKE_CPU_STATUS_SAT_CACHE_MEM);

	return service | (target << 3) | (owner << 5) | (fairness_due << 7);
}

static __always_inline u8
cake_service_readiness_action_for_wake(const struct task_struct *p,
				       u64 target_status, u32 target_cpu,
				       u32 enqueue_cpu, u32 service_kind,
				       bool default_bulk_protected)
{
	u8 idx = cake_service_readiness_index_for_wake(
		p, target_status, target_cpu, enqueue_cpu, service_kind,
		default_bulk_protected);

	return cake_service_readiness_table_action(idx);
}

static __always_inline __maybe_unused bool
cake_service_readiness_apply_wake_action(struct task_struct *p, u32 target_cpu,
					 u32 enqueue_cpu, u64 slice,
					 ...
					 bool default_bulk_protected)
{
	...
	action = cake_service_readiness_action_for_wake(
		p, target_status, target_cpu, enqueue_cpu, service_kind,
		default_bulk_protected);

	if (action == CAKE_READY_ACT_LOCAL_INSERT) {
	...
	if (action == CAKE_READY_ACT_IDLE_KICK &&
	    (target_status & CAKE_CPU_STATUS_IDLE)) {
#if CAKE_HAS_ENQ_KICK_IDLE
		u64 fold = cake_enq_kick_idle_flag;
#endif
		CAKE_GAME_DIAG_INC(target_cpu, enqueue_direct_local);
	...
	if (action == CAKE_READY_ACT_PREEMPT_KICK) {
		CAKE_GAME_DIAG_INC(target_cpu, enqueue_direct_local);
					    ...
					    u32 service_kind)
{
...
#endif
	CAKE_GAME_DIAG_INC(target_cpu, local_waiter_attempt);
	/* Unified local-waiter service insert: futex AND schbench (and any other
	 * admitted handoff service) take ONE path — head-insert plus an explicit
	 * preempt kick so the woken waiter runs now instead of parking behind the
	 * owner's slice. The old futex branch head-inserted but skipped the kick,
	 * leaving the wakee waiting out the owner (the futex-throughput gap vs
	 * EEVDF, which preempts on wake). One decision, no per-service silo. */
	if (!cake_local_waiter_admit_service(p, target_status, service_kind)) {
#ifndef CAKE_RELEASE
		if (lw_stats)
			lw_stats->nr_local_waiter_reject++;
#endif
		CAKE_GAME_DIAG_INC(target_cpu, local_waiter_reject);
		return false;
	}
	...
	if (cake_local_waiter_quench_current(target_cpu, p, target_status)) {
		...
		CAKE_GAME_DIAG_INC(target_cpu, local_waiter_quench);
	}
#else
	if (cake_local_waiter_quench_current(target_cpu, p, target_status))
		CAKE_GAME_DIAG_INC(target_cpu, local_waiter_quench);
#endif
	CAKE_GAME_DIAG_INC(target_cpu, local_waiter_insert);
	cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | target_cpu, slice,
			   enq_flags | SCX_ENQ_HEAD);
	scx_bpf_kick_cpu(target_cpu, SCX_KICK_PREEMPT);
	return true;
			    ...
			    u64 enq_flags, u64 target_status)
{
	...
	if (idle) {
		...
		CAKE_GAME_DIAG_INC(target_cpu, enqueue_wake_idle);
		CAKE_GAME_DIAG_INC(target_cpu, wake_kick_idle);
		/* Redundant-idle-kick ablation (SCX_CAKE_ENQ_SKIP_IDLE_KICK):
		 * the kernel's local_dsq_post_enq() already reschedules an idle
...
#if CAKE_LEAN_SCHED && !CAKE_SELECT_CPU_AND_COMPAT_FASTPATH && \
	!CAKE_SELECT_CPU_AND_STRUCT_FASTPATH
...
static __always_inline bool cake_cpu_external_pressure(u32 cpu)
{
#if CAKE_MAX_CPUS <= 64
	u64 mask;
	...
	if (cpu >= 64)
		return false;
	mask = READ_ONCE(cake_external_pressure_mask);
	return !!(mask & (1ULL << cpu));
#else
	(void)cpu;
	return false;
#endif
...
)
{
	...
	if (cake_cpu_external_pressure(candidate)) {
...
static __always_inline s32 cake_select_cpu_fast_scan(struct task_struct *p,
						s32 prev_cpu, u64 wake_flags,
						struct cake_cpu_bss *local_bss)
{
	cake_fast_probe_pack_t probe_pack;
...
static __noinline __maybe_unused s32
cake_select_stress_status_idle_sweep(struct task_struct *p, s32 prev_cpu,
				     struct cake_cpu_bss *local_bss,
				     u32 local_cpu)
{
	cake_fast_probe_pack_t probe_pack;
	...
	if (local_cpu < ncpus && local_cpu != prev) {
		selected = cake_try_clean_idle_candidate_record(
			local_bss, local_cpu, CAKE_ROUTE_SLOT0);
		if (selected >= 0)
			return selected;
	}
	...
	candidate = cake_fast_probe_slot_from_pack(probe_pack, 0);
	if (candidate < ncpus && candidate != prev && candidate != local_cpu) {
		selected = cake_try_clean_idle_candidate_record(
			local_bss, candidate, CAKE_ROUTE_SLOT0);
		if (selected >= 0)
			return selected;
...
#else
static __always_inline __maybe_unused s32
cake_select_stress_status_idle_sweep(struct task_struct *p, s32 prev_cpu,
				     struct cake_cpu_bss *local_bss,
				     u32 local_cpu)
{
				  ...
				  u16 select_flags)
{
	 ...
	 *
	 * The signal-local vtime-local arm uses the same clean-idle shortcut:
	 * vtime-local still updates ordering/accounting on enqueue, but a reserved
	 * idle CPU has no shared queue to arbitrate and no local waiter to kick. */
	...
	{
		u64 futex_lane_now = 0;

		if (cake_futex_lane_active(&futex_lane_now) &&
			...
			cake_futex_lane_note_now(futex_lane_now);
			p->scx.slice = slice;
			cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | (u32)cpu,
					   slice,
		...
		if (service_kind == CAKE_TASK_SERVICE_STRESS_FUTEX) {
			...
			cake_futex_lane_note_now(futex_lane_now ?: bpf_ktime_get_ns());
			p->scx.slice = slice;
			cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | (u32)cpu, slice,
					   SCX_ENQ_HEAD | SCX_ENQ_IMMED);
			return true;
		}
	}
	...
	cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | (u32)cpu, slice, 0);
	return true;
#else
	(void)p;
	(void)cpu;
...
#define stats_on 0
	...
	if (p->prio >= 120 && p->scx.weight == 100
		...
		if (prev_cpu >= 0 && prev_cpu < cake_nr_cpus
			...
			CAKE_GAME_DIAG_INC(local_cpu, nfw_prev_idle_attempt);
			if (cake_nfw_prev_take_ok((u32)prev_cpu)) {
				if (scx_bpf_test_and_clear_cpu_idle(prev_cpu)) {
					CAKE_GAME_DIAG_INC(local_cpu, nfw_prev_idle_claim);
					nfw_cpu = prev_cpu;
				}
			} else {
				CAKE_GAME_DIAG_INC(local_cpu, nfw_prev_idle_sibling_block);
			}
		}
#else
		if (prev_cpu >= 0 && prev_cpu < cake_nr_cpus &&
...
#if CAKE_GAME_DIAG
		if (nfw_cpu < 0) {
			CAKE_GAME_DIAG_INC(local_cpu, nfw_prev_idle_fallback_attempt);
			nfw_cpu = select_cpu_and_idle(p, prev_cpu, wake_flags, 0);
			if (nfw_cpu >= 0) {
				CAKE_GAME_DIAG_INC(local_cpu, nfw_prev_idle_fallback_hit);
				if (nfw_cpu == prev_cpu)
					CAKE_GAME_DIAG_INC(
						local_cpu,
						nfw_prev_idle_fallback_prev);
				else
					CAKE_GAME_DIAG_INC(
						local_cpu,
		...
		if (nfw_cpu < 0)
			nfw_cpu = select_cpu_and_idle(p, prev_cpu, wake_flags, 0);
#endif
		CAKE_GAME_DIAG_INC(local_cpu, nfw_entry);

		if (nfw_cpu >= 0) {
			u64 nfw_slice = quantum_ns;

			CAKE_GAME_DIAG_INC(local_cpu, nfw_hit);
#if CAKE_GAME_DIAG
			cake_game_diag_count_nfw_hit_comm(p, local_cpu);
			if (prev_cpu >= 0 && prev_cpu < cake_nr_cpus) {
				u32 prev_u = (u32)prev_cpu;
				u16 prev_primary;
				u16 nfw_primary;

				if (nfw_cpu == prev_cpu)
					CAKE_GAME_DIAG_INC(local_cpu,
							   nfw_hit_prev_cpu);
				else
					CAKE_GAME_DIAG_INC(local_cpu,
							   nfw_hit_other_cpu);
				if ((u32)nfw_cpu == local_cpu)
					CAKE_GAME_DIAG_INC(local_cpu,
							   nfw_hit_select_cpu);
				prev_primary = cake_primary_cpu((u16)prev_u);
				nfw_primary = cake_primary_cpu((u16)nfw_cpu);
				if (prev_primary != CAKE_CPU_SENTINEL &&
				    nfw_primary != CAKE_CPU_SENTINEL) {
					if (prev_primary == nfw_primary)
						CAKE_GAME_DIAG_INC(
							local_cpu,
							nfw_hit_prev_primary);
					else
						CAKE_GAME_DIAG_INC(
							local_cpu,
							nfw_hit_other_primary);
				}
			} else {
				CAKE_GAME_DIAG_INC(local_cpu, nfw_hit_other_cpu);
			}
			{
				...
				CAKE_GAME_DIAG_INC(local_cpu,
						   nfw_hit_local_depth_sample);
				if (local_depth > 0)
					CAKE_GAME_DIAG_INC(
						local_cpu,
						nfw_hit_local_depth_nonzero);
				if (local_depth > 1)
					CAKE_GAME_DIAG_INC(local_cpu,
							   nfw_hit_local_depth_gt1);
				if (local_depth > 3)
					CAKE_GAME_DIAG_INC(local_cpu,
							   nfw_hit_local_depth_gt3);
			}
#endif
			CAKE_GAME_DIAG_INC(local_cpu, nfw_direct_insert);
		 ...
		 * cosmos-style work conservation at the frame-overlap edge. */
		{
			...
			CAKE_GAME_DIAG_INC(local_cpu, nfw_miss_shared);
		...
		CAKE_GAME_DIAG_INC(local_cpu, nfw_miss_tunnel);
		goto tunnel;
#endif
		/* Miss: fall through to Cake's guarded machinery. */
		CAKE_GAME_DIAG_INC(local_cpu, nfw_fallthrough);
	}
#endif
...
tunnel:
	 ...
	 * enqueue route the task through the normal per-CPU local path. */
	{
		...
		CAKE_GAME_DIAG_INC(local_cpu, select_tunnel);
#if CAKE_FUTEX_TRACE
		if (cake_task_service_kind(p) == CAKE_TASK_SERVICE_STRESS_FUTEX)
	...
	if (prev_cpu >= 0 && prev_cpu < cake_nr_cpus &&
		...
		if (service_kind == CAKE_TASK_SERVICE_STRESS_FUTEX) {
			...
			p->scx.slice = slice;
			cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | prev_idx,
					   slice,
					   SCX_ENQ_HEAD | SCX_ENQ_IMMED);
			return prev_cpu;
		}
...
static __always_inline void cake_dsq_insert(struct task_struct *p, u64 dsq_id,
					    u64 slice, u64 enq_flags)
{
#if CAKE_RELEASE_LOCAL_DISPATCH && CAKE_DSQ_INSERT_V2_FASTPATH
	scx_bpf_dsq_insert___v2___compat(p, dsq_id, slice, enq_flags);
#else
	dsq_insert_wrapper(p, dsq_id, slice, enq_flags);
#endif
}

static __noinline __maybe_unused void
dsq_insert_vtime_wrapper(struct task_struct *p, u64 dsq_id, u64 slice,
...
static __always_inline __maybe_unused u64 cake_llc_dsq_for_cpu(u32 cpu)
{
#ifdef CAKE_SINGLE_LLC
	(void)cpu;
	return LLC_DSQ_BASE;
...
static __noinline __maybe_unused bool cake_dsq_move_to_local(u64 dsq_id,
							     u64 enq_flags)
{
...
cake_core_steal_pull_primary(u32 primary)
{
	...
	if (likely(cpu_lfdeqs[0] != NULL)) {
		...
		if (p) {
			cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | own_cpu, p->scx.slice, 0);
			bpf_task_release(p);
			hit = true;
		}
					 ...
					 struct task_struct *prev, u32 cpu_idx)
{
	...
	if (likely(cpu_lfdeqs[0] != NULL)) {
		...
		if (sibling < cake_nr_cpus && sibling != idx) {
			struct scx_lfdeq __arena *prim_sib_lfdeq =
				get_cpu_lfdeq(sibling);
			if (prim_sib_lfdeq) {
				u64 h = READ_ONCE(prim_sib_lfdeq->head);
				u64 t = READ_ONCE(prim_sib_lfdeq->tail);
				u64 eq_h = READ_ONCE(prim_sib_lfdeq->eq_buf.head);
				  ...
				  struct task_struct *prev, u32 cpu_idx)
{
	...
	(void)cpu_idx;
	return false;
#elif defined(CAKE_RELEASE) && defined(CAKE_SINGLE_LLC)
	bool hit;
...
static __always_inline __maybe_unused bool
cake_llc_vtime_frame_signal_boost(struct task_struct *p, u64 target_status,
				   bool is_wakeup)
{
	if (!is_wakeup)
		return false;
#if CAKE_LLC_VTIME_SIGNAL_LOCAL && CAKE_HYBRID_QUEUE_VALUE && defined(CAKE_RELEASE)
	u64 comm0 = cake_task_comm_word(p, 0);

	/* WoW A/B signal-local gate (SCX_CAKE_WOW_FRAME_BOOST):
	 * 1: Boost only the WoW.exe main thread, mirroring hybrid4's conditional
	 *    GameThread anchor policy.  Do not boost all WoW.exe workers.
	 * 2: Also boost vkd3d* present/queue/fence helpers to test whether GPU-feed
	 *    userspace submit threads want the same local HEAD/PREEMPT signal. */
#if CAKE_WOW_FRAME_BOOST_VALUE
	if ((comm0 & CAKE_COMM_MASK7) == CAKE_COMM_WOW_EXE &&
	    p->pid == p->tgid) {
		u32 owner = (u32)((target_status >> CAKE_CPU_STATUS_OWNER_SHIFT) &
				  CAKE_CPU_STATUS_OWNER_MASK);

		return (target_status & CAKE_CPU_STATUS_IDLE) ||
		       owner == CAKE_CPU_OWNER_FRAME ||
		       owner == CAKE_CPU_OWNER_INTERACTIVE;
	}
#if CAKE_WOW_FRAME_BOOST_VALUE >= 2
	if ((comm0 & CAKE_COMM_MASK5) == CAKE_COMM_VKD3D_PREFIX)
		return true;
#endif
#endif

	if (CAKE_HYBRID_GATE_RT && comm0 == CAKE_COMM_RENDER_THREAD)
		return true;
	if (CAKE_HYBRID_GATE_GT && comm0 == CAKE_COMM_GAME_THREAD)
		return true;
	if (CAKE_HYBRID_GATE_GT_COND && comm0 == CAKE_COMM_GAME_THREAD &&
	    p->pid == p->tgid) {
		u32 owner = (u32)((target_status >> CAKE_CPU_STATUS_OWNER_SHIFT) &
				  CAKE_CPU_STATUS_OWNER_MASK);
...
static __always_inline __maybe_unused void
cake_insert_llc_vtime_signal_local(struct task_struct *p, u64 enq_flags,
				   u32 target_cpu, u64 slice,
				   u64 target_status, u32 service_kind,
				   bool force_frame_boost)
{
#if CAKE_RELEASE_LOCAL_DISPATCH
	bool is_wakeup = !!(enq_flags & (u64)SCX_ENQ_WAKEUP);
	bool idle = !!(target_status & CAKE_CPU_STATUS_IDLE);
	bool frame_boost = force_frame_boost ||
			   (service_kind == CAKE_TASK_SERVICE_NONE &&
			    cake_llc_vtime_frame_signal_boost(p, target_status,
							      is_wakeup));
	u64 insert_flags = enq_flags;
#if CAKE_HAS_ENQ_KICK_IDLE
	u64 fold = idle ? cake_enq_kick_idle_flag : 0;
#endif
	...
	if (is_wakeup) {
		...
		if (frame_boost)
			...
			kick = cake_busy_wake_kick_from_status_service(
				p, target_status, service_kind, target_cpu);

		if (kick == SCX_KICK_PREEMPT)
			CAKE_GAME_DIAG_INC(target_cpu, wake_kick_preempt);
		else
			CAKE_GAME_DIAG_INC(target_cpu, wake_kick_idle);
...
#if CAKE_RELEASE_LOCAL_DISPATCH
	cake_insert_llc_vtime_signal_local(p, enq_flags, target_cpu, slice,
					   cake_read_cpu_status(target_cpu),
...
static __always_inline __maybe_unused u64 cake_stream_slice(void)
{
...
static __always_inline __maybe_unused void
cake_insert_stream_service(struct task_struct *p, u64 enq_flags,
			   u32 target_cpu, u64 slice, u32 service_kind)
{
#if !CAKE_HAS_DOMAIN_DRR && CAKE_USE_CUSTOM_DSQ_LANES
	cake_stream_service_mark();
	dsq_insert_vtime_wrapper(p, cake_stream_service_dsq(), slice,
				 p->scx.dsq_vtime, enq_flags);
#elif CAKE_LLC_VTIME_SIGNAL_LOCAL
	cake_insert_llc_vtime_signal_local(p, enq_flags, target_cpu, slice,
					   cake_read_cpu_status(target_cpu),
					   service_kind, false);
#else
			       ...
			       u64 slice, u64 enq_flags)
{
	...
	cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | target_cpu, slice, enq_flags);
}
...
#if CAKE_RELEASE_LOCAL_DISPATCH
#if !CAKE_HAS_LLC_PENDING || CAKE_LLC_VTIME_SIGNAL_LOCAL
	(void)cpu;
			       ...
			       struct task_struct *prev, u64 dec)
{
	...
	u32 runs;
	u32 shift;
	bool stream_pressure = !!(dec & CAKE_TP_DEC_STREAM_PRESSURE);
	...
	if (!(dec & CAKE_TP_DEC_SAT_CACHE_MEM))
		return false;
#if CAKE_LLC_VTIME_SIGNAL_LOCAL
	/* Signal-local has no custom cache/stream service DSQ to arbitrate mixed
	 * stress-ng cache+memcpy work.  Preserve the old cache-simple contract in
	 * local DSQ terms: keep the cache-hot owner resident while stream pressure
	 * is merely pending, but stop sprinting once the mixed-stream bleed budget
	 * says a stream turn is due. */
		      ...
		      u64 slice, u32 service_kind)
{
...
#endif
			cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | target_cpu,
					   slice, enq_flags);
			return;
		}
	}
	 ...
	 * 2026-06-17 on the legacy llc-vtime/hybrid4 nightly; opened by the
	 * LOCAL->llc-vtime default flip in 6ca110b39). Marking the pending bit is
...
enqueue_dsq_dispatch(struct task_struct *p, u64 enq_flags, u32 enq_cpu)
{
#if CAKE_LEAN_SCHED
#if CAKE_RELEASE_LOCAL_DISPATCH
	cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | enq_cpu, p->scx.slice,
			   enq_flags);
	return;
#else
	if (likely(CAKE_QUEUE_POLICY == CAKE_QUEUE_POLICY_LLC_VTIME)) {
		 ...
		 * beat) but cedes 1%low/p99. Frame-family wakeups now use
		 * LOCAL HEAD/PREEMPT as a vtime signal; everything else stays
		 * direct-local instead of entering a shared LLC DSQ. */
		u64 comm0 = cake_task_comm_word(p, 0);
		...
		if ((CAKE_HYBRID_GATE_GT &&
			...
			cake_insert_llc_vtime_signal_local(
				p, enq_flags, enq_cpu, p->scx.slice,
				cake_read_cpu_status(enq_cpu),
				CAKE_TASK_SERVICE_NONE, true);
			return;
		}
		 ...
		 * same-named workers fall through to normal local-vtime. */
		if (CAKE_HYBRID_GATE_GT_COND &&
			...
			if ((ts & CAKE_CPU_STATUS_IDLE) ||
				...
				cake_insert_llc_vtime_signal_local(
					p, enq_flags, enq_cpu, p->scx.slice,
					ts, CAKE_TASK_SERVICE_NONE, true);
				return;
			}
		}
#endif
	...
	if (likely(CAKE_QUEUE_POLICY == CAKE_QUEUE_POLICY_LLC_VTIME)) {
		...
		if (CAKE_HYBRID_GATE_GT_COND &&
			...
			if ((hts & CAKE_CPU_STATUS_IDLE) ||
				...
				cake_insert_llc_vtime_signal_local(
					p, enq_flags, enq_cpu, p->scx.slice,
					hts, CAKE_TASK_SERVICE_NONE, true);
				return;
			}
		}
#endif
...
static __always_inline u8 cake_ready_bits_for_wake(
	const struct task_struct *p, u32 service_kind, u32 target_cpu,
	u64 target_status, bool default_bulk_protected)
{
	u8 bits = 0;
	bool urgent =
		service_kind == CAKE_TASK_SERVICE_STRESS_FUTEX ||
		...
		service_kind == CAKE_TASK_SERVICE_PERF_SCHED_PIPE ||
		((p->flags & PF_KTHREAD) && p->prio < 120);
	bool route_known = target_cpu < cake_nr_cpus;
	bool cache_safe =
		service_kind != CAKE_TASK_SERVICE_STRESS_CACHE &&
		service_kind != CAKE_TASK_SERVICE_STRESS_MEMCPY &&
		!(target_status & CAKE_CPU_STATUS_SAT_CACHE_MEM) &&
		!default_bulk_protected;
	bool owner_preempt_ok =
		!!(target_status & CAKE_CPU_STATUS_IDLE) ||
		!!(target_status & CAKE_CPU_STATUS_PREEMPT_WAKE) ||
		p->prio < 120 || p->scx.weight > 120;

	if (urgent)
		bits |= CAKE_READY_URGENT_SERVICE;
	if (!default_bulk_protected)
		bits |= CAKE_READY_FAIRNESS_OK;
	if (owner_preempt_ok)
		bits |= CAKE_READY_OWNER_PREEMPT_OK;
	if (route_known)
		bits |= CAKE_READY_ROUTE_KNOWN;
	if (cache_safe)
		bits |= CAKE_READY_CACHE_DAMAGE_OK;
	if ((target_status & CAKE_CPU_STATUS_IDLE) ||
	    (owner_preempt_ok && urgent))
		bits |= CAKE_READY_KICK_BUYS_SERVICE;

	return bits;
}

static __always_inline bool cake_ready_action_promotes(u8 action)
{
	return action == CAKE_READY_ACT_LOCAL_INSERT ||
	       action == CAKE_READY_ACT_IDLE_KICK ||
	       action == CAKE_READY_ACT_PREEMPT_KICK;
}

static __always_inline u8 cake_ready_action_for_wake(u8 bits, u64 target_status)
{
	if (!(bits & CAKE_READY_ROUTE_KNOWN))
		return CAKE_READY_ACT_SHARED_ESCAPE;
	if (!(bits & CAKE_READY_FAIRNESS_OK))
		return CAKE_READY_ACT_KEEP_OWNER;
	if (!(bits & CAKE_READY_CACHE_DAMAGE_OK))
		return CAKE_READY_ACT_THROUGHPUT_LANE;
	if (target_status & CAKE_CPU_STATUS_IDLE)
		return CAKE_READY_ACT_IDLE_KICK;
	if ((bits & CAKE_READY_URGENT_SERVICE) &&
	    (bits & CAKE_READY_OWNER_PREEMPT_OK) &&
	    (bits & CAKE_READY_KICK_BUYS_SERVICE))
		return CAKE_READY_ACT_PREEMPT_KICK;
	if (bits & CAKE_READY_OWNER_PREEMPT_OK)
		return CAKE_READY_ACT_LOCAL_INSERT;
	return CAKE_READY_ACT_KEEP_OWNER;
}

static __always_inline void cake_ready_record_enqueue(
	struct cake_stats *stats, struct cake_task_ctx __arena *tctx,
	const struct task_struct *p, u32 target_cpu, u32 service_kind,
	u64 target_status, bool default_bulk_protected)
{
	u8 bits;
	u8 action;
	...
	bits = cake_ready_bits_for_wake(p, service_kind, target_cpu,
					target_status, default_bulk_protected);
	action = cake_ready_action_for_wake(bits, target_status);
	...
	stats->nr_ready_eval++;
	if (bits & CAKE_READY_URGENT_SERVICE)
		stats->nr_ready_urgent++;
	if (cake_ready_action_promotes(action))
		stats->nr_ready_promote++;
	else
		stats->nr_ready_defer++;
	if (action == CAKE_READY_ACT_PREEMPT_KICK)
		stats->nr_ready_preempt++;
	else if (action == CAKE_READY_ACT_IDLE_KICK)
		stats->nr_ready_idle_kick++;
	else if (action == CAKE_READY_ACT_LOCAL_INSERT)
		stats->nr_ready_local_only++;

	if (!(bits & CAKE_READY_FAIRNESS_OK))
		stats->nr_ready_defer_fairness++;
	if (!(bits & CAKE_READY_OWNER_PREEMPT_OK))
		stats->nr_ready_defer_owner++;
	if (!(bits & CAKE_READY_ROUTE_KNOWN))
		stats->nr_ready_defer_route++;
	if (!(bits & CAKE_READY_CACHE_DAMAGE_OK))
		stats->nr_ready_defer_cache++;
}
#endif
...
static __noinline void enqueue_body(struct task_struct *p, u64 enq_flags)
{
	...
	u32 enqueue_cpu = bpf_get_smp_processor_id() & (CAKE_MAX_CPUS - 1);
	u64 slice = quantum_ns;
	...
	u32 service_kind = normal_default ? cake_task_service_kind(p) :
					    CAKE_TASK_SERVICE_NONE;
	CAKE_GAME_DIAG_INC(enqueue_cpu, enqueue_call);
	if (enq_flags & (u64)SCX_ENQ_WAKEUP)
		CAKE_GAME_DIAG_INC(enqueue_cpu, enqueue_wakeup);
	else
		CAKE_GAME_DIAG_INC(enqueue_cpu, enqueue_non_wakeup);
	if (enq_flags & (u64)SCX_ENQ_REENQ)
		CAKE_GAME_DIAG_INC(enqueue_cpu, enqueue_requeue);
	if (enq_flags & (u64)SCX_ENQ_PREEMPT)
		CAKE_GAME_DIAG_INC(enqueue_cpu, enqueue_preserve);
	...
	/* Fast enqueue (SCX_CAKE_FAST_ENQUEUE, game A/B): ordinary non-service
	 * default-user wakeups skip the bulk-owner/local-waiter/steal candidacy
	 * machinery — straight local insert plus the status-based busy kick that
	 * carries the tails.  Service contracts stay ahead of this path. */
#if CAKE_FAST_ENQUEUE_VALUE
	if (service_kind == CAKE_TASK_SERVICE_NONE && normal_default &&
	    (enq_flags & (u64)SCX_ENQ_WAKEUP) &&
	    !(enq_flags & ((u64)SCX_ENQ_REENQ | (u64)SCX_ENQ_PREEMPT))) {
		...
		if (fe_status & CAKE_CPU_STATUS_IDLE) {
			...
			CAKE_GAME_DIAG_INC(target_cpu, enqueue_wake_busy);
			if (enqueue_cpu == target_cpu)
				CAKE_GAME_DIAG_INC(target_cpu, enqueue_wake_busy_local);
			else
				CAKE_GAME_DIAG_INC(target_cpu, enqueue_wake_busy_remote);
			kick = cake_busy_wake_kick_from_status_service(
				p, fe_status, CAKE_TASK_SERVICE_NONE, target_cpu);
			if (kick == SCX_KICK_PREEMPT)
				CAKE_GAME_DIAG_INC(target_cpu, wake_kick_preempt);
			else
				CAKE_GAME_DIAG_INC(target_cpu, wake_kick_idle);
	...
	if (service_kind == CAKE_TASK_SERVICE_PERF_SCHED_MESSAGING &&
		...
		if (idle) {
			...
			CAKE_GAME_DIAG_INC(target_cpu, enqueue_wake_busy);
			if (enqueue_cpu == target_cpu)
				CAKE_GAME_DIAG_INC(target_cpu,
						   enqueue_wake_busy_local);
			else
				CAKE_GAME_DIAG_INC(target_cpu,
						   enqueue_wake_busy_remote);
		}
		cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | target_cpu, slice,
#if CAKE_HAS_ENQ_KICK_IDLE
				   enq_flags | fold);
	...
	if (kthread && (prio < 120 || cake_task_is_affinitized(p))) {
		p->scx.slice = slice;
		CAKE_GAME_DIAG_INC(target_cpu, enqueue_direct_local);
		CAKE_GAME_DIAG_INC(target_cpu, kthread_direct_insert);
		cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | target_cpu, slice,
				   enq_flags);
		...
		if (enq_flags & (u64)SCX_ENQ_WAKEUP) {
			...
			if (!(kts & CAKE_CPU_STATUS_IDLE)) {
				CAKE_GAME_DIAG_INC(target_cpu, wake_kick_preempt);
				CAKE_GAME_DIAG_INC(target_cpu, kthread_wake_preempt);
	...
	if (service_kind == CAKE_TASK_SERVICE_STRESS_CACHE) {
		struct cake_cpu_bss *target_bss =
			&cpu_bss[target_cpu & (CAKE_MAX_CPUS - 1)];
		u64 target_dec = READ_ONCE(target_bss->throughput_decision);
		bool cache_simple_ok =
			cake_cache_simple_note_cache() &&
			!cake_mixed_stream_bleed_due_dec(target_bss,
							 target_dec);
		...
		if (cache_simple_ok) {
			...
			/* Local-only cache-simple: when no idle CPU is available,
			 * put the cache worker at the front of the target local DSQ
			 * instead of relying on the old shared cache/simple lane. */
#if CAKE_LLC_VTIME_SIGNAL_LOCAL
			service_flags |= SCX_ENQ_HEAD;
#endif
			cake_insert_llc_vtime(p, service_flags, target_cpu, slice,
					      service_kind);
			return;
		}
	}
	...
	if (service_kind == CAKE_TASK_SERVICE_STRESS_MEMCPY) {
		...
		if (idle_cpu < 0)
			idle_cpu = select_cpu_and_idle(p, (s32)target_cpu, 0,
						       0);
		if (idle_cpu >= 0) {
			cake_dsq_insert(p,
		...
		if (!mixed_cache_active) {
			...
			cake_insert_stream_service(p, enq_flags, target_cpu,
						   slice, service_kind);
			return;
		}
	...
	if (unlikely(p->scx.dsq_vtime == 0)) {
		CAKE_GAME_DIAG_INC(target_cpu, enqueue_initial);
		p->scx.dsq_vtime = cake_read_cpu_frontier(target_cpu);
		p->scx.slice = slice;
#if CAKE_HAS_LOCAL_WAITER
		if ((enq_flags & (u64)SCX_ENQ_WAKEUP) &&
		...
		if (enq_flags & (u64)SCX_ENQ_WAKEUP) {
			...
			if (service_kind == CAKE_TASK_SERVICE_NONE &&
				...
				cake_insert_llc_vtime_signal_local(
					...
					target_status, service_kind, true);
				return;
			}
		}
#endif
...

	{
		...
		if (service_kind == CAKE_TASK_SERVICE_NONE &&
		    cake_llc_vtime_frame_signal_boost(p, target_status,
						      true)) {
			cake_insert_llc_vtime_signal_local(
				p, enq_flags, target_cpu, slice, target_status,
				service_kind, true);
			return;
		}
#endif
		...
		if (cake_service_readiness_apply_wake_action(
			    p, target_cpu, enqueue_cpu, slice, enq_flags,
			    target_status, service_kind, default_bulk_protected))
			return;
#endif
		cake_insert_local_kick_idle(p, target_cpu, slice, enq_flags,
					    target_status);
		/* Busy-kick ablation (SCX_CAKE_ENQ_SKIP_BUSY_KICK): measure the
		 * cost of the busy-wake scx_bpf_kick_cpu kfunc and whether the
		 * preempt-kick matters for FPS. Skipping it leaves the wakee in
		 * the local DSQ until the target reschedules on its own. */
#ifndef CAKE_ENQ_SKIP_BUSY_KICK
#define CAKE_ENQ_SKIP_BUSY_KICK 0
#endif
#if !CAKE_ENQ_SKIP_BUSY_KICK
		if (!(target_status & CAKE_CPU_STATUS_IDLE)) {
			u64 kick;

#if CAKE_KTHREAD_WAKE_PREEMPT_VALUE || CAKE_WAKE_PREEMPT_ELAPSED_VALUE || \
	CAKE_LEAN_WAKE_KICK_VALUE
			kick = cake_busy_wake_kick_from_status_service(
				p, target_status, service_kind, target_cpu);
#else
			/* The service kinds which avoid busy preempt
			 * (sched-messaging/sched-pipe) have already returned through
			 * their direct enqueue contracts above.  The remaining default
			 * wake tail can use the release policy decision directly without
			 * rechecking that unreachable service branch.
			 */
			if ((target_status & CAKE_CPU_STATUS_PREEMPT_WAKE) ||
			    p->prio < 120 || p->scx.weight > 120)
				kick = SCX_KICK_PREEMPT;
...
#define path_stats ((struct cake_stats *)0)
	...
	if ((p->flags & PF_KTHREAD) &&
		...
		cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | target_cpu, quantum_ns,
				   enq_flags);
		if (p->prio >= 120)
			scx_bpf_kick_cpu(target_cpu, SCX_KICK_PREEMPT);
		return;
	...
	if (preserve_state) {
		...
		cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | target_cpu, preserved,
				   enq_flags);
		return;
	}
...
#undef path_stats
	...
	if (stats_on) {
		...
		if (tctx) {
			...
			tctx->telemetry.pending_strict_owner_class =
				CAKE_WAKE_CLASS_NONE;
			tctx->telemetry.pending_target_pressure = 0;
			tctx->telemetry.pending_ready_bits = 0;
			tctx->telemetry.pending_ready_action = CAKE_READY_ACT_NONE;
		}
...
#endif
		cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | task_cpu, quantum_ns,
				   enq_flags);
#ifndef CAKE_RELEASE
		if (stats_on) {
	...
	if (stats_on) {
		u64 ready_status = cake_read_cpu_status(target_cpu);

		cake_ready_record_enqueue(stats, tctx, p, target_cpu,
					  service_kind, ready_status, false);
	}
#endif
...
queue_affine_preserve:
	...
	if (stats_on)
		dsq_insert_start = bpf_ktime_get_ns();
#endif
	cake_dsq_insert(p, SCX_DSQ_LOCAL_ON | target_cpu, p->scx.slice,
			   enq_flags);
	goto queue_done;
...
#define path_stats_on 0
	...
	/* Lean-dispatch rescue skip (SCX_CAKE_DISPATCH_SKIP_RESCUE): the two
	 * idle-rescue work-conservation pulls below measured 0 hits across
	 * 10.5M game dispatches — pure per-dispatch branch+helper cost in the
	 * game regime. Knob compiles them out; default keeps them for loaded
	 * (bench/contended) workloads where the steal/pull does fire. */
			...
			for (u32 off = 1; off < CAKE_MAX_LLCS; off++) {
				...
				if (should_pull &&
					...
					cake_dispatch_record_pull_result(
						dispatch_bss, true);
#endif
					CAKE_GAME_DIAG_INC(cpu_idx,
							   dispatch_llc_pull_hit);
					if (stats_on || path_stats_on) {
						struct cake_stats *s =
							get_local_stats_for(
								cpu_idx);
			       ...
			       u64 start)
{
	...
	ww->ready_bits	  = tctx->telemetry.pending_ready_bits;
	ww->ready_action = tctx->telemetry.pending_ready_action;
...
static __always_inline void
cake_ready_record_action_outcome(struct cake_stats *s_run,
				 struct cake_wake_wait_scratch *ww)
{
	bool promoted;

	if (!s_run || ww->ready_action <= CAKE_READY_ACT_NONE ||
	    ww->ready_action >= CAKE_READY_ACT_MAX)
		return;

	promoted = cake_ready_action_promotes(ww->ready_action);
	if (promoted) {
		if (ww->wait_ns < 100000ULL)
			s_run->nr_ready_promote_wait_lt100us++;
		if (ww->wait_ns >= 1000000ULL)
			s_run->nr_ready_promote_wait_ge1ms++;
	} else {
		if (ww->wait_ns < 100000ULL)
			s_run->nr_ready_defer_wait_lt100us++;
		if (ww->wait_ns >= 1000000ULL)
			s_run->nr_ready_defer_wait_ge1ms++;
	}
}

static __noinline void
running_wake_wait_record_readiness(struct cake_task_ctx __arena *tctx,
				   struct cake_stats	      *s_run,
				   struct cake_wake_wait_scratch *ww)
{
	(void)tctx;
	cake_ready_record_action_outcome(s_run, ww);
}
...
void BPF_STRUCT_OPS(cake_stopping, struct task_struct *p, bool runnable)
{
...
#if CAKE_GAME_DIAG
	CAKE_GAME_DIAG_INC(cpu, stopping_lean_return);
#endif
	return;
#endif
#if CAKE_GAME_DIAG
	CAKE_GAME_DIAG_INC(cpu, stopping_owner_update);
#endif
	u32  owner_avg_runtime_ns =
		cake_update_owner_avg(bss, rt_raw, owner_service_kind);
	...
	bool route_pending = p->pid && (u32)route_pred == p->pid &&
			     (route_pred & CAKE_ROUTE_PRED_PENDING);
#else
	bool route_pending = false;
#endif
#if CAKE_GAME_DIAG
	CAKE_GAME_DIAG_INC(cpu, stopping_route_observe);
	if (route_pending)
		CAKE_GAME_DIAG_INC(cpu, stopping_route_pending);
	else
		CAKE_GAME_DIAG_INC(cpu, stopping_route_no_pending);
#endif
	if (route_pending)
		cake_route_pred_observe(bss, p, rt_raw, runnable);
	cake_publish_cpu_owner(cpu, bss, owner_avg_runtime_ns);
...
#if CAKE_GAME_DIAG
	if (relaxed)
		CAKE_GAME_DIAG_INC(cpu, stopping_account_relaxed);
	else
		CAKE_GAME_DIAG_INC(cpu, stopping_account_audit);
#endif
	if (!relaxed) {
#if CAKE_GAME_DIAG
		CAKE_GAME_DIAG_INC(cpu, stopping_scoreboard_owner_result);
#endif
