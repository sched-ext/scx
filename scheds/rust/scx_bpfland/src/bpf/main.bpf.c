			...
			s32 prev_cpu, u64 wake_flags)
{
	...
	if (is_idle && (local_pcpu || !scx_bpf_dsq_nr_queued(cpu))) {
		       ...
		       s32 prev_cpu, bool idle_smt)
{
	...
	cpu = scx_bpf_pick_idle_cpu(p->cpus_ptr, flags);
	if (cpu >= 0)
		return cpu;
...
#else
	/*
	 * Only allow CPU migrations on wakeup via ops.select_cpu().
	 */
	cpu = prev_cpu;
#endif
