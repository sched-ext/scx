...
static inline u64 scale_by_task_normalized_weight_inverse(const struct task_struct *p, u64 value)
{
	 ...
	 *
	 * The goal is to ensure a more balanced scheduling behavior that
	 * is influenced more by the task's behavior rather than its
	 * priority difference and prevent potential stalls due to large
	 * priority gaps.
	*/
		       ...
		       s32 prev_cpu, bool idle_smt)
{
	 ...
	 *
	 * Only allow migrations within the same L2 or L3 cache.
	 */
	mask = cast_mask(tctx->l2_cpumask);
	if (mask) {
		cpu = __COMPAT_scx_bpf_pick_idle_cpu_node(mask,
					node, flags | __COMPAT_SCX_PICK_IDLE_IN_NODE);
		if (cpu >= 0)
			return cpu;
	}
