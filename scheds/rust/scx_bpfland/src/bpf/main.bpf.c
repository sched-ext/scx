...
/*
 * Maximum rate of task enqueues/sec (tasks with a higher rate are capped
 * to this value).
 */
#define MAX_ENQ_FREQ		1024ULL
...
s32 BPF_STRUCT_OPS(bpfland_select_cpu, struct task_struct *p, s32 prev_cpu, u64 wake_flags)
{
	 ...
	 *
	 * However, if the task has a high enqueue rate, keep it where it
	 * is to reduce the amount of migrations.
	 */
	return (is_this_cpu_allowed &&
		(tctx->enq_freq < MAX_ENQ_FREQ)) ? this_cpu : prev_cpu;
}
...
{
	const struct task_struct *p = __COMPAT_scx_bpf_cpu_curr(cpu);

	return p && p->flags & PF_IDLE;
}

/*
 * Tasks doing a high rate of short sleeps (e.g., `stress-ng --sleep N`)
 * can trigger a intense rate of rescheduling events, causing locking
 * contention in the node DSQs.
 *
 * Prevent this by forcing such tasks to be dispatched to the local DSQ for
 * a certain amount of time (@slice_lag).
 */
...
void BPF_STRUCT_OPS(bpfland_enqueue, struct task_struct *p, u64 enq_flags)
{
	...
	/*
	 * Update the task's enqueue frequency based on the time since the
	 * last wakeup, then cap the result to avoid large spikes.
	 */
	delta_t = now > tctx->last_enq_at ? now - tctx->last_enq_at : 1;
	tctx->enq_freq = update_freq(tctx->enq_freq, delta_t);
	tctx->enq_freq = MIN(tctx->enq_freq, MAX_ENQ_FREQ);
	...
	/*
	 * If the task is marked as sticky or if it gets constantly
	 * re-enqueued, just force it to stay on the local CPU.
	 */
	if (tctx->enq_freq == MAX_ENQ_FREQ ||
	    time_after(tctx->pinned_at + slice_lag, bpf_ktime_get_ns()))
		force_local = true;

	/*
	 * If the previously used CPU is still idle or the task is forced
	 * to stay on its CPU, dispatch it directly to the local DSQ.
	 */
	if (is_cpu_idle(prev_cpu) || force_local) {
		scx_bpf_dsq_insert(p, SCX_DSQ_LOCAL, slice, enq_flags);
		__sync_fetch_and_add(&nr_kthread_dispatches, 1);
		return;
	}
