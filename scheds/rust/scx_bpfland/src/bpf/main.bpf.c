...
static s32 pick_idle_cpu(struct task_struct *p, s32 prev_cpu, u64 wake_flags, bool *is_idle)
{
	...
	if (smt_enabled) {
		 ...
		 */
		cpu = __COMPAT_scx_bpf_pick_idle_cpu_node(l2_mask, node, SCX_PICK_IDLE_CORE | SCX_PICK_IDLE_IN_NODE);
		if (cpu >= 0) {
			*is_idle = true;
			goto out_put_cpumask;
		}
		 ...
		 */
		cpu = __COMPAT_scx_bpf_pick_idle_cpu_node(l3_mask, node, SCX_PICK_IDLE_CORE | SCX_PICK_IDLE_IN_NODE);
		if (cpu >= 0) {
			*is_idle = true;
			goto out_put_cpumask;
		}
		 ...
		 */
		cpu = __COMPAT_scx_bpf_pick_idle_cpu_node(p_mask, node, SCX_PICK_IDLE_CORE);
		if (cpu >= 0) {
			*is_idle = true;
			goto out_put_cpumask;
		}
		 ...
		 */
		cpu = __COMPAT_scx_bpf_pick_idle_cpu_node(p->cpus_ptr, node, SCX_PICK_IDLE_CORE);
		if (cpu >= 0) {
			*is_idle = true;
			goto out_put_cpumask;
		}
	 ...
	 * L2 cache.
	 */
	cpu = __COMPAT_scx_bpf_pick_idle_cpu_node(l2_mask, node, SCX_PICK_IDLE_IN_NODE);
	if (cpu >= 0) {
		*is_idle = true;
		goto out_put_cpumask;
	}
	 ...
	 * L3 cache.
	 */
	cpu = __COMPAT_scx_bpf_pick_idle_cpu_node(l3_mask, node, SCX_PICK_IDLE_IN_NODE);
	if (cpu >= 0) {
		*is_idle = true;
		goto out_put_cpumask;
	}
			  ...
			  s32 prev_cpu, bool idle_smt)
{
	...
	if (mask) {
		cpu = __COMPAT_scx_bpf_pick_idle_cpu_node(mask, node, flags | SCX_PICK_IDLE_IN_NODE);
		if (cpu >= 0) {
