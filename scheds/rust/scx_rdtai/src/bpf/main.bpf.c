 ...
 *
 * scx_rdtai is a multi-domain BPF / userspace hybrid scheduler where the BPF
 * part does simple round robin in each domain and the userspace part
 * calculates the load factor of each domain and tells the BPF part how to load
 * balance the domains.
 *
 * Every task has an entry in the task_data map which lists which domain the
 * task belongs to. When a task first enters the system (rdtai_prep_enable),
 * they are round-robined to a domain.
 *
 * rdtai_select_cpu is the primary scheduling logic, invoked when a task
 ...
 * dispatch queue corresponding to the domain (rdtai_enqueue).
 *
 * rdtai_dispatch will attempt to consume a task from its domain's
 * corresponding dispatch queue (this occurs after scheduling any tasks directly
 * assigned to it due to the logic in rdtai_select_cpu). If no task is found,
...
struct {
	...
	__type(key, u32);
	__type(value, struct rdtai_percpu_storage);
...
static int scx_rdtai_percpu_storage_init(void)
{
	void *map = &scx_percpu_bpfmask_map;
	struct rdtai_percpu_storage *storage;
	const u32 zero = 0;
	int ret, i;
			   ...
			   u64 enq_flags)
{
	 ...
	 * interactivity boosting for rdtai are described below.
	 */
...
s32 BPF_STRUCT_OPS(rdtai_select_cpu, struct task_struct *p, s32 prev_cpu,
		   u64 wake_flags)
{
...
void BPF_STRUCT_OPS(rdtai_enqueue, struct task_struct *p __arg_trusted, u64 enq_flags)
{
	struct task_ctx *taskc;
	dom_ptr domc;
...
void BPF_STRUCT_OPS(rdtai_dispatch, s32 cpu, struct task_struct *prev)
{
...
void BPF_STRUCT_OPS(rdtai_runnable, struct task_struct *p, u64 enq_flags)
{
...
void BPF_STRUCT_OPS(rdtai_running, struct task_struct *p)
{
...
void BPF_STRUCT_OPS(rdtai_stopping, struct task_struct *p, bool runnable)
{
	struct task_ctx *taskc;
	dom_ptr domc;
...
void BPF_STRUCT_OPS(rdtai_set_weight, struct task_struct *p, u32 weight)
{
	struct task_ctx *taskc;
...
void BPF_STRUCT_OPS(rdtai_set_cpumask, struct task_struct *p,
		    const struct cpumask *cpumask)
{
	struct task_ctx *taskc;
...
s32 BPF_STRUCT_OPS_SLEEPABLE(rdtai_init_task, struct task_struct *p,
...
void BPF_STRUCT_OPS(rdtai_exit_task, struct task_struct *p,
...
__weak s32 create_dom(u32 dom_id)
{
	...
	bpf_for(cpu, 0, MAX_CPUS) {
		 ...
		 */
		perf = min(SCX_CPUPERF_ONE, rdtai_perf_mode);
...
s32 BPF_STRUCT_OPS_SLEEPABLE(rdtai_init)
{
	s32 i, ret;
	...
	ret = scx_rdtai_percpu_storage_init();
	if (ret)
		return ret;
...
SCX_OPS_DEFINE(rdtai,
	       .select_cpu		= (void *)rdtai_select_cpu,
	       .enqueue			= (void *)rdtai_enqueue,
	       .dispatch		= (void *)rdtai_dispatch,
	       .runnable		= (void *)rdtai_runnable,
	       .running			= (void *)rdtai_running,
	       .stopping		= (void *)rdtai_stopping,
	       .quiescent		= (void *)rdtai_quiescent,
	       .set_weight		= (void *)rdtai_set_weight,
	       .set_cpumask		= (void *)rdtai_set_cpumask,
	       .init_task		= (void *)rdtai_init_task,
	       .exit_task		= (void *)rdtai_exit_task,
	       .init			= (void *)rdtai_init,
	       .exit			= (void *)rdtai_exit,
	       .timeout_ms		= 10000,
	       .name			= "rdtai");
