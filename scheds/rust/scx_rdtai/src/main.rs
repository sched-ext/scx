...
const SCHEDULER_NAME: &str = "scx_rdtai";
const MAX_DOMS: usize = bpf_intf::consts_MAX_DOMS as usize;
const MAX_CPUS: usize = bpf_intf::consts_MAX_CPUS as usize;

/// scx_rdtai: A multi-domain BPF / userspace hybrid scheduler
///
...
///
/// WARNING: scx_rdtai currently assumes that all domains have equal
...
impl<'a> Scheduler<'a> {
    fn init(opts: &Opts, open_object: &'a mut MaybeUninit<OpenObject>) -> Result<Self> {
        ...
        info!(
            "Running scx_rdtai (build ID: {})",
            build_id::full_version(env!("CARGO_PKG_VERSION"))
        );
        let open_opts = opts.libbpf.clone().into_bpf_open_opts();
        let mut skel = scx_ops_open!(skel_builder, open_object, rdtai, open_opts).unwrap();
        ...
        if opts.partial {
            skel.struct_ops.rdtai_mut().flags |= *compat::SCX_OPS_SWITCH_PARTIAL;
        }
        skel.struct_ops.rdtai_mut().exit_dump_len = opts.exit_dump_len;
        ...
        info!("Rusty scheduler started! Run `scx_rdtai --monitor` for metrics.");
