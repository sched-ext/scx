...
static void collect_sys_stat(void)
{
	...
	bpf_arena_for(cpu, 0, nr_cids) {
		...
		ravg_accumulate_arena(&cpuc->avg_irq_steal_ravg, cpuc->cur_steal_util_invr,
				      c->now, LAVD_RAVG_HALFLIFE_NS);
		u64 avg_irq_fp = ravg_read_arena(&cpuc->avg_irq_steal_ravg, c->now,
