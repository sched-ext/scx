...
const SCHEDULER_NAME: &str = "scx_nitosis";
...
/// scx_nitosis: A dynamic affinity scheduler
///
...
impl<'a> Scheduler<'a> {
    ...
    fn run(&mut self, shutdown: Arc<AtomicBool>) -> Result<UserExitInfo> {
        ...
        info!("Mitosis Scheduler Attached. Run `scx_nitosis --monitor` for metrics.");
...
fn run(opts: Opts) -> Result<()> {
    ...
    if let Some(run_id) = opts.run_id {
        info!("scx_nitosis run_id: {}", run_id);
    }
...
mod tests {
    ...
    fn requires_cell_parent_cgroup_for_scheduler_mode() {
        assert!(Opts::try_parse_from(["scx_nitosis"]).is_err());
    }

    #[test]
    fn allows_monitor_without_cell_parent_cgroup() {
        assert!(Opts::try_parse_from(["scx_nitosis", "--monitor", "1"]).is_ok());
    }

    #[test]
    fn allows_version_without_cell_parent_cgroup() {
        assert!(Opts::try_parse_from(["scx_nitosis", "--version"]).is_ok());
    }
}
