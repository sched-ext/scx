#!/bin/bash
#
# Minimal reproducer for the scx_nitosis LLC drain affinity failure.
#
...
#
# scx_nitosis is started with LLC-awareness and demand rebalancing. Rebalancing
...
CGROUP_BASE="/sys/fs/cgroup/scx_nitosis_repro.$$"
VICTIM_NAME="mitosis_victim"
PRESSURE_NAME="mitosis_pressure"
LOG_FILE="/tmp/scx_nitosis_llc_drain_affinity.log"
MONITOR_LOG_FILE="/tmp/scx_nitosis_llc_drain_affinity_monitor.log"
...
build_helper() {
    TMPDIR="$(mktemp -d /tmp/scx_nitosis_repro.XXXXXX)"
...
if [[ ! -x "$SCHEDULER_BIN" ]]; then
    ...
    error "Build it with: cargo build --release -p scx_nitosis"
    exit 1
fi
...
if ! ps -p "$SCHED_PID" >/dev/null 2>&1; then
    error "scx_nitosis failed to start"
    cat "$LOG_FILE"
    exit 1
fi
...
if ! ps -p "$MONITOR_PID" >/dev/null 2>&1; then
    error "scx_nitosis monitor failed to start"
...
if ! ps -p "$SCHED_PID" >/dev/null 2>&1; then
    ...
    if grep -Eq 'runnable task stall \(mito_victim\[[0-9]+\] failed to run|target CPU .* not allowed|affinity' "$LOG_FILE"; then
        info "PASS: reproduced scx_nitosis LLC drain affinity failure"
        grep -En 'runnable task stall \(mito_victim\[[0-9]+\] failed to run|target CPU .* not allowed|affinity' "$LOG_FILE" | tail -10
        exit 0
    fi

    error "scx_nitosis exited, but not with the expected failure"
    tail -200 "$LOG_FILE"
    exit 1
fi
...
if (( DRAIN_AFFN_CNT <= 0 )); then
    error "scx_nitosis survived, but drain_affn_cnt did not increase"
    tail -120 "$LOG_FILE"
    tail -120 "$MONITOR_LOG_FILE"
    exit 1
fi

info "PASS: scx_nitosis survived after exercising drain affinity rescue (drain_affn_cnt=$DRAIN_AFFN_CNT)"
exit 0
