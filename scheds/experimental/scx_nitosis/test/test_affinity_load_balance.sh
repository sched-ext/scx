...
#
# Strategy:
# 1. Start scx_nitosis in CellManager mode with an empty managed parent
...
if [[ ! -x "$SCHEDULER_BIN" ]]; then
    ...
    echo -e "${RED}Please build with: cargo build --release -p scx_nitosis${NC}"
    exit 1
fi
...
cleanup() {
    ...
    if [[ -n "$SCHED_PID" ]]; then
        ...
        pkill -9 scx_nitosis 2>/dev/null || true
    fi
...
start_scheduler() {
    ...
    "$SCHEDULER_BIN" "${scheduler_args[@]}" > /tmp/scx_nitosis_affinity.log 2>&1 &
    SCHED_PID=$!
    ...
    if ! ps -p "$SCHED_PID" > /dev/null 2>&1; then
        echo -e "${RED}Failed to start scx_nitosis${NC}"
        cat /tmp/scx_nitosis_affinity.log
        exit 1
    fi
