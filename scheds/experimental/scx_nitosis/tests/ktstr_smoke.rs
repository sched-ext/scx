...
//! scx_nitosis cell-isolation smoke test, and a showcase of the ktstr harness.
//!
//! Two tenants, each in its own cgroup → its own mitosis cell: a small
//! "sensitive" tenant and a CPU "hog". Phase 0 runs under scx_nitosis; phase 1
...
/// scx_nitosis in cell-manager mode: each child of `--cell-parent-cgroup` is a cell.
const MITOSIS: Scheduler = Scheduler::named("mitosis")
    .binary_discover("scx_nitosis")
    .sched_args(&[
        "--cell-parent-cgroup",
        "/test.slice",
...
fn mitosis_isolates_sensitive_tenant(ctx: &Ctx) -> Result<AssertResult> {
    let steps = vec![
        // Phase 0: both tenants under scx_nitosis.
