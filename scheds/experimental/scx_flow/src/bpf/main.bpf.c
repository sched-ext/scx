...
enum wake_profile_bits {
	...
	WAKE_PROFILE_LATENCY_ALLOWANCE = 1U << 1,
						   ...
						   struct task_ctx *tctx)
{
	...
	allowance_ready = is_latency_allowance_candidate(tctx);
	pressure_ready = has_urgent_latency_pressure(tctx);
	rt_sensitive_ready = tctx->last_refill_ns > 0 &&
		(p->nr_cpus_allowed == 1 || p->prio < 100) &&
		tctx->last_refill_ns >= (s64)FLOW_INTERACTIVE_FLOOR_MIN_NS;
	preempt_ready = (p->nr_cpus_allowed == 1 ||
						  ...
						  bool latency_lane_wakeup)
{
	...
	if (target_cpu < 0)
		return false;
	if (rt_sensitive_wakeup || latency_lane_wakeup)
		return false;
...
static __always_inline bool should_prioritize_reserved_enqueue(
	const struct task_ctx *tctx, bool is_wakeup,
...
void BPF_STRUCT_OPS(flow_enqueue, struct task_struct *p, u64 enq_flags)
{
	...
	if (tctx && tctx->budget_ns > 0) {
		...
		direct_local_wakeup = is_locality_candidate(
			...
			rt_sensitive_wakeup, latency_lane_wakeup);
		...
		reserved_priority_wakeup = should_prioritize_reserved_enqueue(
			tctx, is_wakeup, latency_lane_wakeup,
